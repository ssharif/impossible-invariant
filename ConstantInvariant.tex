...
%\begin{proof}[Proof of Corollary~\ref{cor:agpol}]
  ...
  Let $\pi_\Phi: \agphi \to \agd$ be the forgetful map that sends $(B,\mu,\phi)$ to $(B,\mu)$. % Thus we may replace $\agd$ and $f$ in the statement of the Corollary with $\agphi$ and $f\pi_\phi$.

  Using the notation in Corollary~\ref{cor:agpol}, let $n = d_g$. Let $\agn$ denote the moduli space for triples $(A,\lambda,L)$, where $A$ is a $g$-dimensional abelian variety, $\lambda$ is a principal polarization on $A$, and $L$ is a level $n$ structure, i.e., a symplectic basis $(P_1,\ldots,P_g,Q_1,\ldots,Q_g)$ for the $n$-torsion $A[n]$. We define morphisms %maps
  \[
  ...
  Note that $\pi_1\pi_2(A,\lambda,L)$ is isomorphic to $(A,\lambda)$.
...
If $(A,\lambda) \in \ag$, let $S_D(A,\lambda)$ denote the set of pairs $(H,K)$ where $H$ and $K$ are  subgroups of $A[n]$ that are isotropic with respect to the pairing induced by $\lambda$, 
  \[
  ...
  %Note that our notion of ``isotropic'' depends on the polarization $\lambda$.
    ...
    Now suppose that $(A_1,\lambda_1), (A_2,\lambda_2) \in \ag$ and $A_1 \weak A_2$. We would like to show that $f_G(A_1,\lambda_1) = f_G(A_2,\lambda_2)$. \textcolor{red}{Unfortunately, the proof doesn't  work.} Observe that $f_G(A_1,\lambda_1) = \sum f(B_1,\mu_1)$, where in the sum $B_1$ runs over quotients of $A_1$ by subgroups $H_1 + K_1$ such that $(H_1,K_1) \in S_D(A_1,\lambda_1)$. (Each $B_1$ appears with some multiplicity, which generically depends only on $D$ and $g$.) Similarly, $f_G(A_2) = \sum (B_2, \mu_2)$ where $B_2$ runs over quotients $A_2/(H_2 + K_2)$ such that $(H_2,K_2) \in S_D(A_2,\lambda_2)$. Identifying $A_1$ and $A_2$ via a weak isomorphism,  then $\lambda_2^{-1} \circ \lambda_1 \in \Aut(A_1)$  induces a bijection
from $S_D(A_1,\lambda_1)$ to $S_D(A_2,\lambda_2)$. \textcolor{red}{Unfortunately this isn't true unless $\lambda_1 = \lambda_2$.}
  Thus for each $B_1$ appearing in the formal sum $f_G(A_1,\lambda_1)$, there is a $B_2$ such that $B_1 \weak B_2$ appearing with the same multiplicity in $f_G(A_2,\lambda_2)$, and conversely. Since $f$ is constant on weak isomorphism classes, so is $f_G$. By Theorem~\ref{thm:invariant-c-constant}, $f_G$ is a constant morphism. Since $\agn$ is connected and both $\pi_2$ and $\pi_\Phi$ are surjective, it follows that $\agd$ is also connected. Thus $f$ is constant.

  Suppose now that $f$ is a rational map. Let $U \subseteq \agd$ be the domain of definition of $f$ and let $U' = (\pi_\Phi\pi_2)^{-1}(U)$. Let $V = \displaystyle\bigcap_{g \in G} g(U')$, i.e., $V$ is the largest Galois-invariant subset of $\agn$ containing $U'$. 
  The proof proceeds as above, if we replace $\agn, \agphi, \agd$, and $\ag$ with $V, \pi_2(V), \pi_\Phi\pi_2(V)$,  and $\pi_1\pi_2(V)$, respectively. The morphism $\pi_1\pi_2$ is Galois, hence \'etale, and therefore open. This implies that $\pi_1\pi_2(V)$ is open in $\ag$. We can define $f_G$ as a rational map defined on $\pi_1\pi_2(V)$, and so can apply Theorem~\ref{thm:invariant-c-constant}. % The morphisms $\pi_1$ and $\pi_1\pi_2$ are Galois, hence \'etale, and therefore they are open maps. Thus $\pi_1(V)$ and $\pi_1\pi_2(V)$ are open sets.
  %Let $V_\phi = \pi_2(V_n)$, let $V = \pi_1(V_\phi)$, and let $V_D = \pi_\phi(V_\phi)$. The maps $\pi_2, \pi_1, \pi_\phi$ are finite and ???, and hence are open. Therefore $V_\phi, V_D$, and $V$ are open. Replacing $\agn, \agphi, \agd, \ag$ with $V_n, V_\phi, V_d, V$, respectively, yields the diagram:
 % \[
 %   \xymatrix{
 %     V_n \ar[d]^{\pi_2} & & \\
...
\begin{remark}
In the proof of Corollary~\ref{cor:agpol}, if the composition $F=f\pi_\Phi\pi_2$  satisfied the property that $F(A_1,\lambda_1,L_1) = F(A_2,\lambda_2,L_2)$ whenever $A_1 \weak A_2$, we could have applied Corollary~\ref{cor:agn} to $F$  and avoided using $\ag$ and the map $f_G$. However, we cannot directly prove that $F$ has this property, since if $\pi_2(A_i,\lambda_i,L_i) = (B_i,\mu_i,\phi_i)$, it is possible that $B_1 \not\weak B_2$.
\end{remark}

\begin{remark}
% To prove Corollary~\ref{cor:agpol}, we cannot simply apply Corollary~\ref{cor:agn}, since if $A_1 \weak A_2$ and $\pi_2(A_i, \lambda_i,L_i) = (B_i, \mu_i, \phi_i)$, it may not be true that $B_1 \weak B_2$. Furthermore, 
The reason we used $\agn$ in the proof of Corollary~\ref{cor:agpol} is that while $\agn/\ag$ is Galois, $\agphi/\ag$ is not Galois, so the strategy of the proof of Corollary~\ref{cor:agn} cannot be directly applied.
\end{remark}
