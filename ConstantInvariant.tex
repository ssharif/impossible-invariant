...
\[
  ...
  \]
Take $P_i' \in A$ such that $nP_i' = P_i$. Let $e_i=n/d_i$. We will show that $\psi(e_iP_i') \in \ker \mu$ for all $i$. Since 
$$
...
for all such $T\in B[n]$, where $e_B: B[n] \times B^\vee[n] \to \bmu_n$ is the Weil pairing.
...
\end{lemma}
\begin{proof}
By \cite[Prop.~16.8]{milne-av}, there is a perfect skew-symmetric pairing %$e^\mu$ 
  on $\ker \mu$ with respect to which $\ker \rho$ is a maximal isotropic subgroup of $\ker \mu$.
  Also by \cite[Prop.~16.8]{milne-av}, if $(B,\mu) \in \agd$, then for every choice of maximal isotropic subgroup $H \subseteq \ker \mu$, the quotient  
