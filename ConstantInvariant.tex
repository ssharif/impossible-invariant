...
In positive characteristic, we have a slightly weaker result:
\begin{theorem}
  Suppose that $k$ is a finite field. Let $f: \ao^g \to X$ be a morphism of $k$-schemes. If $f(A) = f(B)$ whenever $A \cong B$, then $f$ is constant.
\end{theorem}
...
\begin{proof}
  If $(E, C) \in Y_0(N)$ is represented by the point $\tau \in \hh$, then we may take $E = \C/\langle 1, \tau \rangle$ and $C = \langle 1/N \rangle$. We first show that $\psimodt(E,C) \approx \psimod(E,C)$.
...
\]
Let $\tilde{\lambda}$ be the polarization coming from the Siegel alternating form for $\tau A$. Multiplication by $A$ on $\C^g$ induces an isomorphism of polarized abelian varieties
\[
...
\]
for the associated morphism. Thus, if the diagonal entries of $A$ are $d_1, \ldots, d_g$ and for $(E, C) \in Y_0(N)$, we define $E_i$ to be $E/(N/d_i)C$, then
\[
  \psidiag(E,C) = E_1 \times \cdots \times E_g.
\]

\begin{theorem}\label{thm:char-p-version}
  The Zariski closure of $\bigcup_A \im\psidiag$ is $\ao^g$.
\end{theorem}
