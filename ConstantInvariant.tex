...
\begin{lemma}\label{lem:finite-fixed-trace}
 For any fixed number $t$, there are finitely many $g \times g$ symmetric positive semi-definite half-integral matrices $Q$ such that $\tr(Q) \leq t$.
\end{lemma}
\begin{proof}
  Let $Q$ be a symmetric positive semi-definite half-integral matrix with $\tr(Q) \leq t$, and let $\lambda_1,\dots,\lambda_g$ be the eigenvalues of $Q$. Note that $\lambda_i \geq 0$, so
  \[
    \sum_{1 \leq i,j \leq g} (Q_{i,j})^2 = \tr(Q^2) = \sum \lambda_i^2 \leq \left(\sum \lambda_i\right)^2 = \tr(Q)^2 \leq t^2.
  \]
...
\begin{lemma}\label{lem:unique-minimizer}
  Let $S$ be any nonempty subset of symmetric positive semi-definite half-integral $g \times g$ matrices $Q$. Then there exists an open cone of positive definite real symmetric $g \times g$ matrix $A$ such that $\displaystyle\min_{Q \in S}\tr(AQ)$ is achieved by a unique $Q$.
\end{lemma}
\begin{proof}
  Since $\{Q \in S \colon \tr(Q) \leq t\}$ is finite for all $t \in \R$,
...
\begin{proof}[Proof of Theorem~\ref{thm:curves-dense}]
  ...
  Let $f$ be a Siegel modular form on $\ag$. Recall that $f$ admits a $q$-expansion of the form $f = \sum_Q c_Q q^{\tr(Q)}$ where $Q$ ranges over all symmetric half-integral positive semi-definite matrices \cite[Ch.~V]{faltings1990degeneration}.
  ...
  Let $n = \min \{\tr(AQ) \colon Q \in S\}$. Then there is a unique $Q_0 \in S$ such that $\tr(AQ_0) = n$. If $k = \C$, one has that the $q$-expansion of $f \circ \psimod$ is $\sum_Q c_Q q^{\tr(AQ)}$. By functoriality, this holds when $k$ is an arbitrary algebraically closed field. By construction, the coefficient of $q^n$ is $c_{Q_0} \neq 0$. Hence $f \circ \psimod \neq 0$.
\end{proof}
