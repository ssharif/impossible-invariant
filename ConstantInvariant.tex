...
\begin{proof}[Proof of Theorem~\ref{thm:curves-dense}]
  ...
  Let $f$ be a Siegel modular function on $\ag$, viewed as a global section of the structure sheaf. Then by Proposition~\ref{prop:q-expansion}, there exist coefficients $c_Q \in R$ for $Q \in \sD$, such that for all $A \in \detl$, $\psimod^*(f)$ admits a $q$-expansion $\sum_{Q \in \sD} c_Q q^{\tr(AQ)}$.
