...
\begin{proof}
 ...
 For $i \ge 1$, let $\sC_i$ be any subgroup of $E_i$ of order $\ell$ other than $\pi_{i-1}(\sC_{i-1})$, and let $\pi_i$ be the quotient map $E_i \to E_i/\sC_i =: E_{i+1}$. We will show inductively that $\End(E_{i+1}) \cong \Z + \ell^{i+1}\sO_{i}$. Since $\End(E_{i}) \cong \Z + \ell^{i}\sO_{K}$, the ring $\End(E_{i})$ has exactly one ideal of norm $\ell$, namely $\ell\End(E_{i})$. This ideal corresponds to the dual of $\pi_{i-1}$. Hence by Lemma~\ref{lemma:order-ideal-subgroup}, $\ker\pi_i$ is not an ideal subgroup, so $\End(E_{i+1})$ must be strictly smaller than $\End(E_{i})$ .  Now $\End(E_{i+1}) \cong \Z + \ell^{i+1}\sO_{i}$ by Theorem~\ref{thm:kani-20b}.
