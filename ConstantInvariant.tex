...
\begin{lemma}\label{lem:K-exists}
  Suppose $\ell$ is a prime and $g$ is a positive integer. Suppose $K$ is an imaginary quadratic field such that $\sO_K^\times = \{\pm 1\}$ and such that $\ell$ splits into principal primes $\alpha\sO_K \cdot \overline{\alpha}\sO_K$ with $\alpha\in \sO_K$.
%  \begin{enumerate}
%    \item $\sO_K^\times = \{\pm 1\}$,
...
\begin{lemma}\label{lem:c-torsor}
  Suppose $E$ is an elliptic curve over an algebraically closed field, and $\End(E) \cong \sO_K$ for some imaginary quadratic field $K$. Let $q$ be a prime inert in $K$ and not equal to the characteristic of the base field. Then the set of subgroups of $E$ of order $q$ is a $\Gt_q$ torsor.
\end{lemma}
\begin{proof}
