...
\subjclass[2010]{Primary: 14K05, Secondary: 11G10,11G15}
...
Our results were motivated by and shed light on a proposed construction of a cryptographic protocol for multiparty non-interactive key exchange.
\end{abstract}
...
inert in $K$, we have $q_i\ne \ell$ for all $i$. The condition on $\car(k)$ splitting ensures that there is an elliptic curve $E$ over $k$ such that $\End(E) \cong \sO_K$.
...
\begin{proposition}\label{prop:q-expansion}
  ...
  Then there is a function  $c : \sD \to R$ such that for all 
