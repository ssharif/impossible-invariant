...
Suppose $(B,\mu) \in \agd$. For the perfect skew-symmetric pairing %$e^\mu$ 
  ...
  for every choice of maximal isotropic subgroup $H \subseteq \ker \mu$, by \cite[Prop.~16.8 and Remark~16.9]{milne-av}, the quotient  
  $B/H$ is an abelian variety that has a principal polarization $\lambda$ such that $\rho^*(\lambda) = \mu$, where $\rho : B \to B/H$ is the projection map. There is then an appropriate choice of level structure $L$ such that $\pi_D(B/H, \lambda, L) \cong (B,\mu)$, so $\pi_D$ is surjective.
