...
\begin{remark}
In the proof of Corollary~\ref{cor:agpol}, the reason we used $\agn$ is that $\agphi$ is not Galois over $\ag$, while $\agn$ is. We were not able to use  the composition $F=f\pi_\Phi\pi_2$  and avoid using $\ag$ and the map $f_G$ since $F$ does not satisfy the property that $F(A_1,\lambda_1,L_1) = F(A_2,\lambda_2,L_2)$ whenever $A_1 \approx A_2$, so we could not apply Theorem~\ref{thm:invariant-c-constant} to $F$.
\end{remark}
