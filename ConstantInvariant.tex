...
\begin{remark}
  Over $\C$, there is an easier proof of Theorem~\ref{thm:curves-dense}. Recall the map $\hh \to \hh_g$ given by $\tau \mapsto \tau A$. Let $V$ be the union of the images of these maps as $A$ varies. By Lemma~\ref{lem:rggt-dense}, the topological closure of $V$ in $\hh_g$ contains the totally imaginary locus; that is, the set of matrices of the form $iA$, where $A$ is a real, symmetric, positive definite matrix. But any holomorphic function which vanishes on the totally imaginary locus must be zero. In particular, the image of $V$ in $\ag$ is Zariski dense.
\end{remark}
