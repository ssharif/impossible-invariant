...
\end{lemma}
\begin{proof}
Suppose $(B,\mu) \in \agd$. By \cite[Theorem 1]{mumford1966}, for the perfect skew-symmetric pairing %$e^\mu$ 
  on $\ker \mu$ defined on p.~293 of \cite{mumford1966}, 
  for every choice of maximal isotropic subgroup $H \subseteq \ker \mu$, %by \cite[Prop.~16.8 and Remark~16.9]{milne-av}, 
  the quotient $B/H$ is an abelian variety that has a principal polarization $\lambda$ such that $\rho^*(\lambda) = \mu$, where $\rho : B \to B/H$ is the projection map. By \cite[Proposition 1]{mumford1966}, there is then an appropriate choice of level structure $L$ such that $\pi_D(B/H, \lambda, L) \cong (B,\mu)$, so $\pi_D$ is surjective.
