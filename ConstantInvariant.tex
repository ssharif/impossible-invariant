...
Theorem~\ref{thm:invariant-c-constant} can also be viewed as making precise the discussion in~\cite[p. 97]{git} about  ``pathologies'' that arise when considering the relation $\approx$.
...
Theorem~\ref{thm:invariant-c-constant} implies the same result even if we impose level structure:
\begin{corollary}\label{cor:agn}
  Let $g,n \in \Z_{\geq 2}$, and let $\agn$ be the moduli space of principally polarized abelian varieties with full level $n$ structure. Suppose that $R$ is a domain with $n$ invertible in $R$, and $f: \agn \dashrightarrow X$ is a rational map of $R$-schemes. Denote points of $\agn$ as pairs $(A,L)$, where $A$ is a principally polarized abelian variety and $L$ is a symplectic basis for the $n$-torsion $A[n]$. Suppose $f((A_1, L_1)) = f((A_2, L_2))$ whenever $A_1 \approx A_2$. Then $f$ is a constant function.
\end{corollary}

\begin{proof}
  Let $\pi: \agn \to \ag$ be the canonical map; that is, the map that ``forgets'' the level $n$ structure. The space $\agn$ is Galois over $\ag$; let $G$ be the Galois group, so that if $R$ is an algebraically closed field, then $G = \Sp_{2g}(\Z/n\Z)$. Let $X^G$ be the product of $\# G$ copies of $X$, indexed by $G$. The group $G$ acts on $X^G$ by permuting the factors: for $x \in X^G$, $h,g \in G$, if the $g$th coordinate of $x$ is $x_g$, then the $g$th coordinate of $h(x)$ is $x_{h^{-1}g}$.
  Let $S_G(X)$ be the quotient of $X^G$ by $G$.

  Let $V$ be the domain of definition of $f$, and let $U = \pi(V)$, which is an open subset of $\ag$. Define a map $f_G: U \to S_G(X)$ as follows. For $x \in U$, choose  $y_1 \in \pi^{-1}(x)$, and for $g \in G$ let $y_g = g(y_1)$. Observe that $\pi^{-1}(x) = \{y_g\}_{g \in G}$. Let $\tilde{z} = (f(y_g))_{g \in G} \in X^G$, and let $z$ be the corresponding point of $S_G(X)$. Then define $f_G(x) = z$.

  Since on the level of moduli, $\pi((A,L)) = A$, we certainly have $f_G(A_1) = f_G(A_2)$ whenever $A_1 \approx A_2$, $A_1,A_2 \in U$. By Theorem~\ref{thm:invariant-c-constant}, $f_G$ is constant. It follows that $f$ is constant.
\end{proof}

The case of non-principally polarized abelian varieties immediately follows.
\begin{corollary}
  Let $g \in \Z_{\geq 2}$, and let $\overline{d} = (d_1, \ldots, d_g)$ where the $d_i$ are positive integers satisfying $d_1 \mid \cdots \mid d_g$. Let $\agd$ be the moduli space of abelian varieties with a polarization of type $\overline{d}$. Suppose that $R$ is a domain with $d_g$ invertible in $R$ and suppose $f: \agd \dashrightarrow X$ is a rational map of $R$-schemes. Denote points of $\agd$ as pairs $(A,\lambda)$, where $A$ is an abelian variety and $\lambda$ is a polarization of type $\overline{d}$. Suppose $f((A_1, \lambda_1)) = f((A_2, \lambda_2))$ whenever $A_1 \approx A_2$. Then $f$ is a constant function.
\end{corollary}

\begin{proof}
  Let $n = d_g$. There is a surjective morphism $\pi_d: \agn \to \agd$ defined as follows. For $(A,L) \in \agn$, let $L = (P_1, \ldots, P_g, Q_1, \ldots, Q_g)$. Let $B$ be the quotient of $A$ by the subgroup generated by the $P_i$ and by $d_iQ_i$ for all $i$. The image of the $Q_i$ generates a subgroup isomorphic to $\Z/d_1\Z \times \cdots \times \Z/d_g\Z$, and $n$ times the principal polarization on $A$ descends to give a polarization $\lambda$ of type $\overline{d}$ on $B$. Define $\pi_d((A,L)) = (B, \lambda)$.

  Consider $(A_1, L_1), (A_2, L_2)$ in the domain of definition of $f \circ \pi_d$. If $A_1 \approx A_2$, then certainly $\pi_d(A_1) \approx \pi_d(A_2)$. Rest?
\end{proof}

We now derive Theorem \ref{thm:sg-c-dense} from Theorems \ref{thm:Sg-dense} and \ref{thm:curves-dense}, and we derive Theorem \ref{thm:invariant-c-constant} from Theorem \ref{thm:sg-c-dense}.
