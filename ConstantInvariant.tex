...
\begin{proof}[Proof of Corollary~\ref{cor:agpol}]
  Let $n = d_g$. Let $\agphi$ be the moduli space for triples $(A,\lambda,\phi)$ where $A$ is an abelian variety, $\lambda$ is a type $d$ polarization on $A$, and $\phi$ is an injective homomorphism
  \[
    \phi: \Z/d_1\Z \times \cdots \times \Z/d_g\Z \to \ker \lambda.
  \]
  ...
  \]
  defined as follows. For $(A,\lambda,L) \in \agn$, define $B$ as the quotient of $A$ by the subgroup $\langle P_1,\ldots,P_g,d_1Q_1,\ldots,d_gQ_g \rangle$. Let $R_i$ be the image of $Q_i$ in $B$. Let $\psi: A \to B$ be the quotient map. Since $\ker(\psi) \subset A[n]$, there exists an isogeny $\rho: B \to A$ such that $\rho \circ \psi = [n]$, the multiplication-by-$n$ map. Define $\lambda'$ to be the pull-back polarization $\rho^*\lambda$. Define $\phi$ by sending the $i$th standard basis vector to $R_i$. Since $R_i \in \ker \rho$, we see that $R_i \in \ker \lambda$. Define $\pi_2(A,\lambda,L) = (B,\lambda',\phi)$.

  For $\pi_1$, given $(B,\lambda',\phi) \in \agphi$, let $A = B/\im(\phi)$. Then $\lambda'$ descends to a unique polarization $\lambda$ on $A$. Set $\pi_1(B,\lambda',\phi) = (A,\lambda)$.
  Let $\pi = \pi_1\pi_2$. One checks that $\pi(A,\lambda,L) = (A,\lambda)$.

  Let $\simg$ be the symmetric quotient of $X^G$, where $G$ is the Galois group for $\agn/\ag$. Geometrically closed points of $\simg$ can be viewed as degree $\# G$ zero cycles on $X$. We define a map $f_G: \ag \to \simg$ as follows. For $z \in \ag$, fix $y \in \agn$ for which $\pi(y) = x$. Define $f_G(z) = \sum_{g \in G} (f\pi_\phi\pi_2)(gy)$.
  ...
  Observe that $f_G(A_1,\lambda_1) = \sum f(B_1,\lambda_1')$, where in the sum the $B_1$ vary over quotients of $A_1$ by subgroups $H_1 + H_2$, $(H_1,H_2) \in S_d(A_1,\lambda_1)$. (Note that each $B_1$ will appear with some multiplicity, which generically depends only on $d$ and $g$.) Similarly, $f_G(A_2) = \sum (B_2, \lambda_2')$ where the $B_2$ varies over quotients $A_2/(H_1' + H_2')$, $(H_1',H_2') \in S_d(A_2,\lambda_2)$. Fix a weak isomorphism between $A_1$ and $A_2$. Then $\lambda_2^{-1} \circ \lambda_1 \in \End(A_1) = \End(A_2)$ induces a bijection
  \[
    S_d(A_1,\lambda_1) \to S_d(A_2,\lambda_2).
  \]
  Therefore for each $B_1$ appearing in $f_G(A_1,\lambda_2)$, there is a $B_2$ appearing in $f_G(A_2,\lambda_2)$ for which $B_1 \approx B_2$, and conversely. Since $f$ is constant on weak isomorphism classes, $f_G$ is also. By Theorem~\ref{thm:invariant-c-constant}, $f_G$ is a constant morphism. The space $\agn$ is connected and $\pi_2$ is surjective, so $\agphi$ is also connected. Therefore $f$ is constant.

  If $f$ is a rational map, we repeat the above proof, but replacing moduli spaces with the appropriate domains of definition.
\end{proof}
