...
%\]
  as follows. If $(A,\lambda,L) \in \agn$, let $B$ denote the quotient of $A$ by the subgroup generated by $\{ P_1,\ldots,P_g,d_1Q_1,\ldots,d_gQ_g \}$ and let $\psi: A \to B$ be the quotient map. Since $\ker(\psi) \subseteq A[n]$, there exists an isogeny $\rho: B \to A$ such that the map $\rho \circ \psi$ is multiplication by $n$. Let $\mu$ be the pull-back polarization $\rho^*(\lambda)$. By \cite[Prop.~16.8]{milne-av}, there is a perfect skew-symmetric pairing %$e^\mu$ 
  ...
  $B/H$ is an abelian variety that has a principal polarization $\lambda$ such that $\rho^*(\lambda) = \mu$. There is then an appropriate choice of level structure $L$ such that $\pi_D(B/H, \lambda, L) \cong (B,\mu)$, and so $\pi_D$ is surjective.
