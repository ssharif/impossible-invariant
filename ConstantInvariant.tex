...
from $S_d(A_1,\lambda_1)$ to $S_d(A_2,\lambda_2)$.
  Thus for each $B_1$ appearing in the formal sum $f_G(A_1,\lambda_2)$, there is a $B_2$ such that $B_1 \approx B_2$ appearing with the same multiplicity in $f_G(A_2,\lambda_2)$, and conversely. Since $f$ is constant on weak isomorphism classes, so is $f_G$. By Theorem~\ref{thm:invariant-c-constant}, $f_G$ is a constant morphism. Since $\agn$ is connected and both $\pi_2$ and $\pi_\phi$ are surjective, it follows that $\agd$ is also connected. Thus $f$ is constant.

  Suppose now that $f$ is a rational map. Let $U \subseteq \agd$ be its domain of definition and let $U_n = (\pi_\phi\pi_2)^{-1}(U)$. Let $V_n = \displaystyle\bigcap_{g \in G} g(U_n)$ that is, $V_n$ is the largest Galois-invariant subset of $\agn$ containing $U_n$. 
  The proof proceeds as above, if we replace $\agn, \agphi, \agd, \ag$ with $V_n, \pi_2(V_n), \pi_\Phi\pi_2(V_n), \pi_1\pi_2(V_n)$, respectively, and note that the morphisms $\pi_2, \pi_1, \pi_\Phi$ are finite and ???
...
\begin{remark}
In the proof of Corollary~\ref{cor:agpol}, the reason we were not able to avoid going to $\agn$ is that $\agphi$ is not Galois over $\ag$, while $\agn$ is. We were not able to use only the maps $\pi_2$ and $\pi_\Phi$ and avoid using $\ag$ and the map $f_G$ since the composition $F=f\pi_\Phi\pi_2$ does not satisfy the property that $F(A_1,\lambda_1,L_1) = F(A_2,\lambda_2,L_2)$ whenever $A_1 \approx A_2$, so we could not apply Theorem~\ref{thm:invariant-c-constant} to $F$.
\end{remark}
