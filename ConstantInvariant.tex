...
\end{theorem}
Furthermore, all of the results stated in this section hold even when $k$ is replaced by a geometrically reduced scheme; see Remark~\ref{rmk:replace-with-scheme}.
% \begin{theorem}\label{thm:sg-c-dense}
...
\begin{proposition}\label{prop:q-expansion}
  Suppose  $g \in \Z_{\ge 1}$, $\ell$ is invertible in the base ring $R$,
  and $f$ an $R$-valued Siegel modular form on $\ag$.
  %Let $\infty \in X_0(N)$ be the unique cusp with type $I_1$ reduction.
  Then there is a function  $c : \sD \to R$ such that for all % but finitely many
