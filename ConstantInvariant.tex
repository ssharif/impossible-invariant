...
\begin{definition}
   Let $D$ denote the set of $g \times g$ symmetric, half-integral, positive semidefinite matrices. If $f$ is a Siegel modular form on $\ag$, let $\psimod^*(f) = f \circ \psimod$ be the pullback modular form on $X_0(N)$.
\end{definition}

\begin{lemma}\label{lemma:q-expansion}
  Suppose $\ell$ is a prime number not equal to the characteristic of the base, $g \in \Z_{\ge 1}$, $f$ a modular form on $\ag$. Let $\infty \in X_0(N)$ be the unique cusp with type $I_1$ reduction. Then there exist coefficients $c_Q$ such that for all $A \in \detl$, the $q$-expansion of the modular form $\psimod^*(f)$ at $\infty$ is of the form
  \[
