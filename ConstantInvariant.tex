...
\begin{proof}[Proof of Lemma~\ref{lem:lim-degree}]
  ...
  For each $i$, we fix a cyclic subgroup $\sC_i$ of $E$ with order $c_i$. We will define $x_1 = (E/\sC_1,\varphi^{\ast}_{\sC_1})$. Before defining $x_2,x_3,\dots$, we will first describe a set of $y$ such that $\psi(x_1) = \psi'(y)$.

  Recall from Section~\ref{sec:geo-desc} the description of $\psi = \psi_{\vec{n},\lambda}$. Because we have the decomposition of $\varphi^{\ast}_{\sC_1}$ into a sequence of $\ell$-isogenies, the geometric description allows us to evaluate
  \[
    \psi(E/\sC_1,\varphi^{\ast}_{\sC_1}) = (E/\varphi^{n_1}(\sC_1) \times \cdots \times E/\varphi^{n_g}(\sC_1),\lambda).
  \]
  The set of $c_1$-torsion subgroups of $E$ is a $(\sO_K/c\sO_K)^\times/(\Z/c\Z)^\times \cong \F_{c_1^2}^\times/\F_{c_1}^\times$ torser by Lemma~\ref{c-torsor}. Recall from our construction of the $c_i$ that $\varphi \mod{c_i\sO_K}$ is a $g$th power. So we can find some $\gamma \in \F_{c_1^2}^\times$ such that $\varphi \equiv \gamma^g \mod{c_1\sO_K}$. In particular, $\varphi(\sC_1) = \gamma^g(\sC_1)$.
...
\begin{proof}
  ...
  \]
  Note that $f \supseteq \alpha \Leftrightarrow f = g\alpha$ for some $g \in \End E$. Identifying $\End E \cong \sO_K$, this means $f \in \alpha\sO_K$. The condition $f(\sC) = \sC$ means that $f \in \sO$. Thus we may identify the set
  \[
    \left\{ f \in \End E \colon \ker f \supseteq \ker \alpha \text{ and } f(\sC) = \sC \right\}
  \]
  with the set
  \[
    \left\{ f \in \alpha\sO_K \colon f \in \sO \right\} = \alpha\sO_K \cap \sO.
  \]
\end{proof}
...
\end{lemma}
\begin{proof}
  Let $I_c(\sO_K)$ denote the monoid of $\sO_K$-ideals prime to $c$. The map $\psi: I_c(\sO_K) \to I_c(\sO)$ sending $\frak{a} \mapsto \frak{a} \cap \sO_K$ is an isomorphism by \cite[Prop.~7.20]{cox2011primes}. The inverse map is $\psi^{-1}(\frak{b}) = \frak{b}\sO_K$.

  After multiplying both sides by $\overline{\beta}$, it is enough to consider the case where $\beta = 1$. That is, we want to show that $\alpha\sO_K \cap \sO$ is principal if and only if $\alpha$ is an integer modulo $c\sO_K$. The latter is equivalent to $\alpha \in \sO$. By applying $\psi^{-1}$, we see that $\alpha\sO_K \cap \sO = \gamma\sO$ for some $\gamma \in \sO$ if and only if $\alpha\sO_K = \gamma\sO_K$. By our hypothesis that $\sO_K^\times = \{\pm 1\}$, this is equivalent to $\alpha = \pm \gamma$, i.e. $\alpha \in \sO$.
\end{proof}
