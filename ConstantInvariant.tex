...
\begin{proof}[Proof of Theorem~\ref{thm:invariant-c-constant}]
  ...
  % \end{tikzcd}
  % \]
  The hypotheses on $f$  imply that $\sg(\C) \subset \df(\C)$.
  % https://ncatlab.org/nlab/show/separated+morphism+of+schemes
   Since $\df$ is a closed subscheme of $\ag \times \ag$, Theorem~\ref{thm:sg-c-dense} implies that $\df = \ag \times \ag$, whence the claim follows.\textcolor{red}{TODO: whence? I think this needs more.}
\end{proof}
