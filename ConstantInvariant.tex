...
\begin{theorem}[Koecher]
  Let $f$ be a holomorphic function on $\hh_g/\Sp_{2g}(\Z)$, with Fourier series $f(\tau) = \sum_N a_N \exp(2 \pi i)^{\tr(\tau N)}$, where $N$ runs over all symmetric half-integer matrices. If $g \geq 2$, then $a_N = 0$ for any $N$ that is not positive semi-definite.
\end{theorem}
\begin{proof}
  See the proof of \cite[Thm.~1, Pg.~45]{klingen1990introductory}.
  %See also https://pdfs.semanticscholar.org/b1f4/d21acdbb599fe177fb15e087dcda2b7c2905.pdf
\end{proof}

In what follows we assume $g \geq 2$.
