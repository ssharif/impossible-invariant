...
hold
if we replace the algebraically closed field $k$ with a geometrically reduced scheme $S$; for example, if we replace $k$ with (Spec of) an integral domain. To see this, note that when $S$ is geometrically integral, we can use the fact that the generic point is dense in $S$. For $S$ geometrically reduced, instead consider the union of the generic points of the irreducible components. To generalize the results below that assume non-divisibility by the characteristic of $k$ (for example, $\car(k) \nmid n$), we replace this hypothesis with the assumption that the value %($n$ in the example) 
...
Suppose $(E,C) \in Y_0(N)$, and let $\psimod(E,C) = (B,\lambda)$. Then $\lambda$ is a principal polarization.  Let $d_1, \cdots, d_g$ be the elementary divisors of $A$, and for $1 \le i \le g$ let $E_i = E/(N/d_i)C$. Then
$
...
\begin{proof}
  For the first claim, we have that $\lambda_A = \pi^\vee \circ \lambda \circ \pi$ where $\pi^\vee$ is the dual isogeny, and hence $\deg(\lambda_A) = \deg(\pi)^2 \cdot \deg(\lambda)$. Therefore $\lambda$ is a principal polarization.
  
  For the second claim, let $D$ be the diagonal matrix with $d_1, \cdots, d_g$ on the diagonal.
%   \[
%     D =
%    \begin{bmatrix}
%      {d_1} & & & \\
