...
\begin{lemma}\label{lem:K-exists}
  ...
  \begin{enumerate}
    ...
    \item there are infinitely many rational primes $q$ such that
    \begin{enumerate}
...
\end{lemma}
\begin{proof}
  By Lemma~\ref{lem:silly}, we can find a $K$ satisfying (1) and (2). Let $L = K(\zeta_g,\alpha^{1/g},\overline{\alpha}^{1/g})$. 
  ...
  Since $\sigma|_K$ is complex conjugation, $q$ is inert in $K$, giving (a). 
  Since $\sigma|_{\Q(\zeta_g)}$ is complex conjugation, $q \equiv -1 \mod{g}$, giving (b).
...
Since $\sigma$ has order $2$, we have $\sO_L/\frak{q} \cong \sO_K/q\sO_K \cong \F_{q^2}$. In particular, in $(\sO_K/q\sO_K)^\times$, we have that $\alpha^{1/g}$ is a $g$th root of $\alpha$, giving (c). 
...
\begin{lemma}\label{lem:c-torsor}
  Suppose $E$ is an elliptic curve over an algebraically closed field, and $\End(E) \cong \sO_K$ for some imaginary quadratic field $K$. Let $q$ be a prime inert in $K$ and not equal to the characteristic of the base field. Then the set of subgroups of $E$ with order $q$ is a $\Gt_q$ torsor.
\end{lemma}
\begin{proof}
