...
Suppose that $g\in\Z_{>0}$, $R$ is a domain,  $D=(d_1,\ldots,d_g)$ is a polarization type, $d_g$ is invertible in $R$, and $n = d_g$. Define a morphism 
  $$\pi_D:\agn \longrightarrow \agd$$
  as follows. If $(A,\lambda,L) \in \agn$ with $L=(P_1,\ldots,P_g,Q_1,\ldots,Q_g)$, let $B$ denote the quotient of $A$ by the subgroup generated by $\{ P_1,\ldots,P_g,d_1Q_1,\ldots,d_gQ_g \}$ and let $\psi: A \to B$ be the quotient map. Since $\ker(\psi) \subseteq A[n]$, there exists an isogeny $\rho: B \to A$ such that the map $\rho \circ \psi$ is multiplication by $n$. Let $\mu$ be the pull-back polarization $\rho^*(\lambda) = \rho^\vee \circ \lambda\circ\rho$. 
...
for all such $T\in B[n]$, where $e_B: B[n] \times B^\vee[n] \to \mu_n$ is the Weil pairing.
Since $\rho(B[n]) = \ker(\psi)$, and $e_B(T,\rho^\vee \lambda(e_iP_i))=e_A(\rho(T),\lambda(e_iP_i))$,
where $e_A: A[n] \times A^\vee[n] \to \mu_n$ is the Weil pairing, 
it suffices to show that
$e_A(P_j,\lambda(e_iP_i))=1$ and $e_A(d_jQ_j,\lambda(e_iP_i))=1$
for all such $i$ and $j$. This follows from the facts that $d_ie_i=n$ and $L$ is a symplectic basis with respect to the pairing induced by $e_A$ and $\lambda$.
...
\end{lemma}
\begin{proof}
Suppose $(B,\mu) \in \agd$. For the perfect skew-symmetric pairing %$e^\mu$ 
  on $\ker \mu$ defined above  \cite[Prop.~16.8]{milne-av}, 
  for every choice of maximal isotropic subgroup $H \subseteq \ker \mu$, the quotient  
  $B/H$ is an abelian variety that has a principal polarization $\lambda$ such that $\rho^*(\lambda) = \mu$, by \cite[Prop.~16.8]{milne-av}. There is then an appropriate choice of level structure $L$ such that $\pi_D(B/H, \lambda, L) \cong (B,\mu)$, so $\pi_D$ is surjective.
...
%If $k$ is an algebraically closed field that contains the base ring $R$, define $\mg(k)  \subseteq (\agn \times \agn)(k)$ %by
%\[
...
\[
\sgd(k) :=  \{((B_1,\mu_1), (B_2, \mu_2)) \in \agd(k) \times \agd(k) \mid B_1 \weak B_2 \} \subseteq (\agd \times \agd)(k)
\]
...
%Therefore the preimage of the closure of $\sgd(k)$ is $\agn \times \agn$.  Since the morphism $\pi_D$ is surjective, $\pi_D \times \pi_D$ is also surjective, and %we have $(\pi_D\times\pi_D)(\mg(k)) = \sgd(k)$.
  %the claim follows.
  Since $\pi_D:\agn \longrightarrow \agd$ is surjective, it follows that $\sgd(k)$ is Zariski dense in $\agd \times \agd$.
\end{proof}
