...
Let $J' \subset \ker \mu'$ be a subgroup such that $J \oplus J' = \ker \mu'$. Necessarily $J'$ is an isotropic subgroup isomorphic to $\Z/e_1\Z \times \cdots \times \Z/e_g\Z$. Let $I = \lambda'\psi^\vee(J')$.
\begin{lemma}
  $I \subset A[n]$ is isotropic for $e_n^\lambda$.
\end{lemma}

\begin{proof}
  Let $T, T' \in I$. There exist $R,R' \in J'$ such that $\lambda'\psi^\vee(R) = T$ and $\lambda'\psi^\vee(R') = T'$. Then
  \begin{align*}
    e_n^\lambda(T,T') &= e_n(T, \lambda(T')) \\
                    ...
                    &= e_B(\mu'(R), R').
  \end{align*}
  But $R \in J' \subset \ker \mu'$, and hence the pairing is trivial.
\end{proof}
