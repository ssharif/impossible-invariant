...
\textcolor{red}{I don't see how the proof implies the statement.}
The map $\langle A,B \rangle \mapsto \tr(AB^t)$ is an inner product on $M_{g \times g}(\R)$, and the half-integral matrices form a discrete subset. So it is enough to show that for $Q$ symmetric positive semi-definite and half-integral, $\tr(Q^2)$ is bounded. Let $\lambda_i$ denote the eigenvalues of $Q$, with multiplicity. By hypothesis, $\lambda_i \in \R$ and $\lambda_i \geq 0$ for all $i$. Then $\tr(Q^2) = \sum \lambda_i^2 \leq (\sum \lambda_i)^2 = \tr(Q)^2 \leq t^2$.
\end{proof}
