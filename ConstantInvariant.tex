...
\begin{theorem}\label{thm:curves-dense-a1}
  The set $\bigcup_A X^{(1)}$ is Zariski dense in $\ao^g$.
\end{theorem}
...
\begin{proof}
 ...
 \]
Thus, $S \subset \bigcup X^{(1)}$. Since $S$ is an infinite set of non-isomorphic elliptic curves, $S$ is dense in $\ao$. Therefore $S^g$ is dense in $\ao^g$.
\end{proof}
