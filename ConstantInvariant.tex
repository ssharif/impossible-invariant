...
\begin{definition}\label{def:gellmatrix}
...
\section{Defining curves $X_{A}$ on $\ag$}
...
Suppose $(E, C) \in Y_0(N)$, i.e., $E$ is an elliptic curve and $C \subset E$ is a cyclic subgroup of $E$ of order $N$.  The matrix $A$ induces a natural endomorphism $\lambda_A$ of $E^g$. Since $A$ is symmetric and positive definite, by identifying $E^g$ with its dual we can view $\lambda_A$ as a polarization on $E^g$ of degree $N^2$. Let
$$
...
%    \end{bmatrix}
%  \]
Then $D$ is the Smith normal form of $A$, so there exist $U,V \in \Gl_g(\Z)$ such that $A = UDV$. For $M \in \Gl_g(\Z)$, let $\rho_M$ denote the natural endomorphism of $E^g$ induced by $M$. Then $\rho_U$ and $\rho_V$ are isomorphisms, and $\rho_A = \rho_U \rho_D \rho_V$. Further,
  \begin{align*}
    \ker \rho_D &= E[d_1] \times E[d_2] \times \cdots \times E[d_g]
  \end{align*}
  and so $(\ker \rho_D) \cap C^g = \prod_{i=1}^g d_i^{-1}NC$. %d_1^{-1}NC \times \cdots \times d_g^{-1}NC$.
  Furthermore, $\rho_V(C^g) = C^g$, so
  \[
    \rho_V((\ker \rho_A) \cap C^g) = (\ker \rho_D) \cap C^g.
  \]
  Thus the quotients of $E^g$ by $(\ker \rho_A) \cap C^g$ and by $(\ker \rho_D) \cap C^g$ are weakly isomorphic. The claim follows.
\end{proof}

\subsection{Analytic description}
\label{sec:ana-desc}

In this section we work over $\C$. Suppose that $A$ is a symmetric, positive definite integer matrix with $\det A = N \neq 0$. Let $\hh$ denote the complex upper half plane and let $\hh_g$ denote the degree $g$ Siegel upper half space. Define %a map
\[
  \psi_A: \hh \to \hh_g, \quad  \tau \mapsto \tau A.
\]

\begin{proposition}\label{prop:psi-A-n}
...
\begin{proof}
Recall that $\Sp_{2g}(\Z)$ (resp., $\Gamma_0(N)$) acts on $\hh_g$ (resp., $\hh$) by fractional linear transformations,
and $Y_0(N) = \hh/\Gamma_0(N)$ and $\ag = \hh_g/\Sp_{2g}(\Z)$. Let
\[
...
\subsection{Equality of the maps $\psimodt$ and $\psimod$}
\label{sec:geo-desc}

In fact, $\psimodt$ is none other than $\psimod$, from which one can obtain an alternative proof of Proposition \ref{prop:psi-A-n}.
\begin{proposition}
...
\begin{proof}
  ...
  \]
Then $\psimodt(\tau) \approx \C^g/\tla$, and
%  \[
...
%  \]
 ...
 Since $\Lambda \subset \Lambda_A$, there is a natural isogeny
  ...
  \]
  This is the same isogeny $\rho_A$ as in the proof of Proposition \ref{prop:psimod-weakly-isomorphic-to-product}; i.e., it is the natural isogeny $E^g \to E^g$ induced by the matrix $A$. Then
\[
...
\begin{lemma}
  Let $S$ be any nonempty subset of symmetric positive semi-definite half-integral $g \times g$ matrices $Q$. Then there exists a positive definite real symmetric $g \times g$ matrix $A$ such that $\min_{Q \in S}\tr(AQ)$ is achieved by a unique $Q$.
\end{lemma}
\begin{proof}
  Define
  \begin{align*}
  ...
  \end{align*}
  Note that for any $t$, $S_{\leq t}$ is finite by Lemma~\ref{lem:bound-trace-finite}.

  Consider the set of symmetric matrices $E$ such that
  \begin{enumerate}
    \item $E$ is small \textcolor{red}{TODO: fix}
    \item $\tr(EQ_i) \neq \tr(EQ_j)$ for all $Q_i,Q_j \in S_{\leq t_1}$.
  \end{enumerate}

  If $E$ is small enough, then $I + E$ is positive definite. Note that if $\epsilon > \max\{|E_{i,j}|\}$ then
  \begin{align*}
    \tr(Q)(1 - \epsilon) \leq \tr((I + E)Q) \leq \tr(Q)(1 + \epsilon).
  \end{align*}
  So if $\tr(Q) > t_1$ and $1 - t_0/t_1 > \epsilon$, then $\tr(Q)(1 - \epsilon) > t_0$. Thus if $E$ is small enough, then $\tr(QE) > t_1$ for any $Q$ with $\tr(Q) > t_1$. We can also ensure $\tr(Q(I+E)) < t_1 - t_0$ for any $Q \in S_{\leq t_1}$. In particular, $\tr(Q(I+E))$ is minimized only when $\tr(Q) = t_0$.

  To see that the second condition can be satisfied, note that the equation $\tr(EQ_i) \neq \tr(EQ_j)$ is a closed linear condition on the entries of $E$. So the set of possible $E$ is the complement of a finite union of hyperplanes in $\R^{g(g+1)/2}$.

  We have shown properties (1) and (2) are both open conditions. Hence there is an open subset of $\R^{g(g+1)/2}$ of possible $E$ around $0$.
