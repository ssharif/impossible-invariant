...
Fix a prime number $\ell$ not equal to the characteristic of $k$. We have
$$
\bigcup_{A,A' \in \detl} \left(\sg(k) \cap (X_A \times X_{A'})\right) =  \sg(k) \cap \left(\bigcup_{A,A' \in \detl} (X_A \times X_{A'})\right)
   \subset \sg(k).
   $$
By Theorem~\ref{thm:Sg-dense}, the set
$\bigcup_{A,A' \in \detl} \left(\sg(k) \cap (X_A \times X_{A'})\right)$ is Zariski dense in
$$
...
\begin{proof}[Proof of Theorem~\ref{thm:invariant-c-constant}]
  ...
  The universal property of $\df$ says that for any scheme $W$ and pair of morphisms $h_1,h_2$ from $W \to \ag$ such that $f \circ h_1 = f \circ h_2$, there is a unique morphism $h: W \to \df$ such that $h_1,h_2$ factor through $h$.

  Let $k$ be an algebraically closed field containing $R$.
  Applying the universal property with $W = \Spec k$ shows that $\df(k)$ consists of pairs $(A,B)$ of abelian varieties over $k$ such that $f(A) = f(B)$.
  By the hypothesis on $f$, if $A \approx B$ then $f(A) = f(B)$. Hence $\sg(k) \subseteq \df$.
  Since $\df$ is a closed subscheme of $\ag \times \ag$\textcolor{red}{TODO: does this need a reference?}, Theorem~\ref{thm:sg-c-dense} implies that $\df = \ag \times \ag$, as desired.
\end{proof}
...
The plan is to find a suitable infinite set of pairs $(x,y) \in \sg(k) \cap (X_A \times X_{A'})$. We will explicitly construct these pairs by choosing $x,y \in \ag$ which are products of isogenous CM curves. To ensure both that our products are weakly isomorphic and that there are enough pairs, we first establish basic observations about ideals in CM fields.
...
\begin{proof}
  ...
  For any imaginary quadratic field $K$, there are at most $2|\sO_K^\times| \leq 12$ algebraic integers of norm $\ell$.
  Therefore $\#K \cap L \leq 12$.
  If $\ell \geq 13$, then $4\lfloor 2\sqrt{\ell} \rfloor > 24$. So there is some $\alpha \in L$ that does not lie in $\Q(i)$ or $\Q(\sqrt{-3})$.
  Hence $K = \Q(\alpha)$ satisfies the claim.
  The remaining values of $\ell < 13$ can be checked by hand.
\end{proof}

\begin{lemma}\label{lem:K-exists}
  Suppose $\ell$ is a prime number, $g$ is a positive integer, and $K$ is an imaginary quadratic field over which $\ell$ splits into principal prime ideals, namely $\ell = \alpha\sO_K \cdot \overline{\alpha}\sO_K$ with $\alpha\in \sO_K$.
%  \begin{enumerate}
%    \item $\sO_K^\times = \{\pm 1\}$,
...
\begin{theorem}[{\cite[Thm.~20b]{kani2011products}}]\label{thm:kani-20b}
  ...
  $\End(E) \subset \End(E/H)$. Moreover, if $H_1$ and $H_2$ are ideal subgroups of $E$, then $E/H_1 \approx E/H_2$ if and only if $I(H_1)$ and $I(H_2)$ are isomorphic as $\End(E)$-modules.
  %$f_{E_H} \mid f_E$
\end{theorem}

\begin{lemma}\label{lem:c-end}
  Suppose $E$ is an elliptic curve over an algebraically closed field, and $\End(E) \cong \sO_K$ for some imaginary quadratic field $K$. Suppose $q$ is a product of distinct prime numbers that are inert in $K$ and not equal to the characteristic of the base field, and suppose $\sC$ is a subgroup of $E$ of order $q$. Then $\End(E/\sC) \cong \Z + q\sO_K$, and every endomorphism of $E/\sC$ is induced by an endomorphism of $E$ that takes $\sC$ to $\sC$.
\end{lemma}
\begin{proof}
...
%    E/\sum_{i=1}^n\sC_i.
%  \]
  Let $\sO_j$ be the order $\End(E_j)$ in $K$, and let $f_j$ be its conductor. Since the isogeny $\pi_j$ has degree $q_j$, we have either $[\sO_j : \sO_{j-1}] = q_j$, or $[\sO_{j-1} : \sO_j] = q_j$, or $\sO_{j-1} = \sO_j$ \cite[Prop.~5]{kohel1996endomorphism}. Therefore $[\sO_K : \sO_j]$ divides $\prod_{i=1}^{j}q_i$. Recall from \cite[Prop.~7.20]{cox2011primes} that ideals of $\sO_j$ that are prime to $f_j$ are in bijection with primes of $\sO_K$ avoiding $f_j$. In particular, there is no prime of $\sO_{j-1}$ of norm $q_j$. So $\ker\pi_j$ is not an ideal subgroup \cite[Prop.~23]{kani2011products}. Then by Theorem~\ref{thm:kani-20b}, $[\sO_{j-1} : \sO_j] = q_j$. It follows that $[\sO_K : \sO_n] = \prod_{i=1}^n q_i$ as required.
\end{proof}
...
\end{lemma}
\begin{proof}
  Note that $\alpha(\sC) = (\alpha\overline{\beta})(\beta(\sC))$ so after replacing $\beta(\sC)$ with $\sC$ and $\alpha\overline{\beta}$ with $\alpha$, it suffices to prove the claim for $\beta = 1$.
...
If $D$ is a subgroup of $E$, let $\pi_{D}$ denote the quotient map $E \to E/D$. Note that $\alpha$ induces an isogeny $\tilde{\alpha}: E/\sC \to E/\alpha(\sC)$ such that $\tilde{\alpha}\circ\pi_{\sC} = \pi_{\alpha(\sC)} \circ \alpha$. The map $\tilde{u}\circ\tilde{\alpha}$ is an endomorphism of $E/\sC$. By Lemma~\ref{lem:c-end}, $\tilde{u}\circ\tilde{\alpha}$ is induced by some $\alpha' \in \sO_K$ such that $\alpha'(\sC) = \sC$. That is, $\pi_{\sC}\circ\alpha' = \tilde{u}\circ\tilde{\alpha}\circ\pi_{\sC} = \tilde{u}\circ\pi_{\alpha(\sC)}\circ\alpha$.
...
\end{lemma}
\begin{proof}
   Let $M$ be the diagonal matrix with diagonal entries $\alpha_1^{-1},\dots,\alpha_{g-1}^{-1}, \alpha_g^{-1}\alpha$. Note that $M\mod{q\sO_K} \in \Sl_g(\sO_K/q\sO_K)$. By \cite[Cor.~5.2, p.~18]{ktheory1964bass}, there exists $M' \in \Sl_g(\sO_K)$ such that $M \equiv M' \mod{q\sO_K}$. The matrix $M'$ corresponds to an automorphism of $E^g$ that sends $\prod_{i=1}^g \alpha_i(\sC)$ to $\sC^{g-1} \times \alpha(\sC)$. A similar construction with $\beta$ shows that
  \[
    \prod_{i=1}^g E/\alpha_i(\sC) \approx \prod_{i=1}^g E/\beta_i(\sC)
    \,\Leftrightarrow\,
...
Recall the map $\psimod$ defined in \eqref{def:XAdef}.
...
Suppose $(E,C) \in Y_0(N)$, and let $\psimod(E,C) = (B,\lambda)$.  Let $d_1, \cdots, d_g$ be the elementary divisors of $A$, and for $1 \le i \le g$ let $E_i = E/d_i^{-1}NC$. Then
$
...
\begin{lemma}\label{lem:lim-degree}
  Let $\ell$ be a prime not equal to the characteristic of the base. (What base?? Should this condition be moved down to the proof, where we can choose $E$ over a field of char not $\ell$?) Suppose $A,A' \in \detl$, $\det(A) = \ell^n$, and $\det(A') = \ell^m$.
...
\begin{proof}
  ...
  \]
  Let $\alpha_{\sC,n}: E/\sC \to E/\alpha^n(\sC)$ be the composition of this chain. Then $\alpha_{\sC,n}$ is a cyclic $\ell^n$-isogeny, so the pair $(E/\sC,\alpha_{\sC,n})$ defines a point in $Y_0(\ell^n)$.
...
{NOTE: We need to resolve the  conflict of notation here, since $A$ means something else in the statement of Theorem \ref{thm:arbit-char}.)
...
\]
where  $E_i := E/d_i^{-1}NC$.
Let $$X^{(1)}_A = \psidiag(Y_0(N)).$$

\begin{theorem}\label{thm:curves-dense-a1}
  Fix a prime number $\ell$. Let $W$ denote the set of all $g \times g$ diagonal matrices with diagonal entries $\ell^{n_1},\dots,\ell^{n_g}$ for some positive integers $n_1,\dots,n_g$.
  The set $\bigcup_{A\in W} X^{(1)}_A$ is Zariski dense in $\ao^g$.
\end{theorem}
\textcolor{red}{Proof does not choose $\ell$ arbitrarily.}
\begin{proof}
 Let $E_0$ be an elliptic curve with CM by $\sO_K$ for some imaginary quadratic field $K$; such a curve is defined over every algebraically closed field $k$ containing the base ring $R$. Choose a prime $\ell$ that is inert in $K$ and fix a subgroup $\sC_0$ of $E_0$ of order $\ell$. Let $\pi_0$ denote the quotient map $E_0 \to E_0/\sC_0 =: E_1$. Since $\sC_0$ is not a kernel ideal of $E_0$, by Theorem~\ref{thm:kani-20b} it follows that $\End(E_1) \cong \Z + \ell\sO_K$.

 For $i \ge 1$, let $\sC_i$ be any subgroup of $E_i$ of order $\ell$ other than $\pi_{i-1}(\sC_{i-1})$, and let $\pi_i$ be the quotient map $E_i \to E_i/\sC_i =: E_{i+1}$. Again, $\End(E_{i+1}) \cong \Z + \ell^{i+1}\sO_{i}$. This is because $\End(E_{i})$ has exactly one ideal of norm $\ell$ given by $\ell\End(E_{i-1})$. This ideal corresponds to the dual of $\pi_{i-1}$. Hence $\ker\pi_i$ is not an ideal subgroup so $\End(E_{i+1})$ must be strictly smaller than ?? by Theorem~\ref{thm:kani-20b}.

 For all $n \in \Z_{\ge 1}$, the composition $\pi^{(n)} = \pi_{n-1} \circ \cdots \circ \pi_0$ is a cyclic $\ell^n$-isogeny on $E$. To see this, note that if $\ker\pi^{(n)}$ were not cyclic, then $\pi^{(n)}$ would factor into a composition of the form $\psi\circ[\ell^m]$, where $[\ell^m]$ is the multiplication by $\ell^m$ map. However, the index of $\End(E_n)$ in $\sO_K$ is then bounded by $\deg\psi$. By above, $\End(E_n)$ has index $\ell^n = \deg\pi^{(n)}$ and therefore
 %we must have that
 $m = 0$.

 Let $S = \{E_i\}_{i=0}^\infty$. Let $(E_{n_1},\dots,E_{n_g}) \in S^g$ and let $A$ be the diagonal matrix with diagonal entries $\ell^{n_1},\dots,\ell^{n_g}$.  Choose any $n > \max\{n_i\}$, and let $\sC = \ker \pi^{(n)}$. Then $(E_0,\pi^{(n)}) \in Y_0(\ell^n)$ and
 \[
...
\begin{theorem}\label{thm:Sg-dense-a1}
  Suppose $g\in\Z_{\ge 2}$, $k$ is an algebraically closed field, and $\ell$ is a prime number not equal to the characteristic of $k$. If $A,A' \in \detl$ and are diagonal, then $\sg^{(1)}(k) \cap (X^{(1)}_A \times X^{(1)}_{A'})$ is Zariski dense in $X^{(1)}_A \times X^{(1)}_{A'}$.
\end{theorem}
...
%In ?? we will compute pull backs of modular forms. It is convenient (instead, say what we mean?) to reduce the computation [what computation?] to one over $\C$.
...
Suppose that $A$ is a symmetric, positive definite integer matrix and $N = \det A$.
Then the map \[
  \psic: \hh \to \hh_g, \quad  \tau \mapsto \tau A
\]
%descends to 
...
Suppose that $A$ is a symmetric, positive definite integer matrix and
...
Since $\psimod(E,C) \approx E^g/K(A)$, we have proven that $\psimod(E,C) \approx \psimodt(E,C)$.
  ...
  A polarization on a $g$-dimensional complex abelian variety $\C^g/(\Z^g + \Omega \Z^g)$ with $\Omega \in \hh_g$ is given by an alternating Riemann form $E: \C^g \times \C^g \to \R$ as in \cite{}. The map $E$ is the unique map satisfying $E(u,\Omega v) = u^tv$ for all $u,v \in \Z^g$. 
...
Now consider the (nonprincipal) polarization $\rho_A$ on $E^g = \C^g/\Lambda$. The corresponding alternating Riemann form is given by $E_A(u,\tau v) = u^tAv$ for all $u,v \in \Z^g$. Since $E_A(\Lambda_A \times \Lambda_A) \subset \Z$, it follows that $E_A$ is an alternating Riemann form for $\C^g/\Lambda_A$ as well. 
%Write $\lambda$ for the polarization of $\C^g/\Lambda_A$ given by $E_A$. 
Since the polarization $E_A$ descends from the product polarization, and the polarization coming from $\psimod$ is the unique polarization descending from the product polarization on $E^g$. we have
\[
  \psimod(E,C) = (\C^g/\Lambda_A, E_A).
\]
%Let $\tilde{\lambda}$ be 
Let $\tilde{E}$ denote the %polarization coming from the 
...
$\tilde{E}(u, \tau A v) = u^tv$ for all $u,v \in \Z^g$. If $u, v \in \Z^g$, then
\begin{align*}
  \tilde{E}(u, \tau A v) &= u^tv \\
                          &= E_A(A^{-1}u, \tau v) \\
                          &= E_{A}(A^{-1}u, A^{-1}\tau Av).
\end{align*}
Thus multiplication by $A$ on $\C^g$ induces an isomorphism of polarized abelian varieties
\[
  \psimod(E,C) = (\C^g/\Lambda_A, E_A) \xrightarrow{\sim} (\C^g/\tla, \tilde{E}) = \psimodt(E,C),
\]
...
\begin{lemma}\label{lemma:q-expansion}
  Suppose $g \in \Z_{\ge 1}$, $f$ is a modular form on $\ag$, and $A$ is a $g \times g$ symmetric, positive definite integral matrix with determinant $N \neq 0$. Let $D$ denote the set of $g \times g$ symmetric, half-integral, positive semidefinite matrices. Let $\psimod^*(f) = f \circ \psimod$ be the pull back modular form on $X_0(N)$. Then for some cusp of $X_0(N)$, the $q$-expansion of the modular form $\psimod^*(f)$ at that cusp is of the form
  \[
...
\begin{proof}
 ...
 %  The morphism $\psimod$ induces a morphism $X_0(N) \to \overline{\ag}$ on the toroidal compactifications. We consider $\psimod$ in a formal neighborhood of the $I_1$ cusp of $X_0(N)$. % Here, $\Delta: \G_m \to \G_m^g$ is the diagonal map, and $\lambda_A: \G_m^g \to \G_m^g$ is the natural map induced by $A$.
 %  Let $E$ be the split torus with character group $S^2(Y)$ and $F$ the split torus with character group $S^2(X)$; see~\cite[IV.2.6]{faltings1990degeneration}. These split tori are essentially parameter spaces for the periods, so $E$ is analogous to the upper half-plane $\hh$ and $F$ to the Siegel upper half-space $\hh_g$. Then $\psimod$ induces a map $E \to F$ characterized by the property that the pull back of $q_{ij}$ is $q^{A_{ij}}$. Since $q$-expansions are compatible with pull backs, the $q$-expansion of $\psimod^*(f)$ is
 %  \begin{align*}
 %    \sum_{Q \in D} c_Q \prod q^{A_{ij}Q_{ij}} &= \sum_{Q \in D} c_Q q^{\sum A_{ij}Q_{ij}} \\
 %    &= \sum_{Q \in D} c_Q q^{\tr(AQ)}.
 %  \end{align*}
...
\begin{proof}[Proof of Theorem~\ref{thm:curves-dense}]
  We claim that $\cup X_A$ is Zariski dense in $\ag$. The claim is trivial when $g = 1$, so we assume $g \geq 2$. It suffices to show that for all non-zero Siegel modular functions $f: \ag \to k$, there exists $A \in \detl$ such that the pull back $\psimod^*(f)$ is non-zero.

  Let $f$ be a Siegel modular function on $\ag$, viewed as a global section of the structure sheaf. By Lemma~\ref{lemma:q-expansion}, $f$ admits a $q$-expansion of the form $f = \sum_{Q \in D} c_Q q^{\tr(Q)}$.
  ...
  Let $n = \min \{\tr(AQ) \colon Q \in S\}$. By the choice of $A$, there is a unique $Q_0 \in S$ such that $\tr(AQ_0) = n$. % If $k = \C$, one has that the $q$-expansion of $f \circ \psimod$ is $\sum_Q c_Q q^{\tr(AQ)}$. By functoriality, this holds when $k$ is an arbitrary algebraically closed field.
  Since $q$-expansions interact
  with modular morphisms in a natural way, %as expected,
  the $q$-expansion of $\psimod^*(f)$ is $\sum_Q c_Q q^{\tr(AQ)}$.   By construction, the coefficient of $q^n$ is $c_{Q_0} \neq 0$. Hence $\psimod^*(f) \neq 0$, as desired.
\end{proof}
