...
\thanks{We thank the Alfred P.~Sloan Foundation and the National Science Foundation, which provided support for the research, and the June 2019 ICERM workshop on Arithmetic of Low-Dimensional Abelian Varieties, where this project began.}

\begin{document}

\begin{abstract}
We show that if a %morphism 
...
%Theorem~\ref{thm:invariant-c-constant} 
...
The following result, which we prove in \S\ref{sec:Cor15proof} below, is a generalization of Theorem \ref{thm:invariant-c-constant} with $\ag$ replaced by the moduli space of  abelian varieties with a fixed %non-principal 
polarization type. 
...
\begin{proof}[Proof of Corollary~\ref{cor:alldcor}]
 ...
 Fix two polarization types $D$ and $D'$. Our goal is to show that $f(\agd) = f(\agdp)$. Choose an elliptic curve $E$ defined over $R$. Then $E^g$ has polarizations $\mu$ and $\mu'$ defined over $R$ of types $D$ and $D'$, respectively, so $(E^g, \mu) \in \agd(R)$ and $(E^g,\mu') \in \agdp(R)$. Since $E^g \weak E^g$, by our hypothesis we have $f(E^g, \mu) = f(E^g, \mu')$. Thus $f(\agd) = f(\agdp)$, as desired. 
  %\textcolor{red}{What if $E^g$ is not in domain of definition?}
\end{proof}
...
\begin{lemma}\label{lem:K-exists}
  Suppose $\ell$ is a prime number, %$g$ is a positive integer, 
...
%To see this, note that under the usual isomorphism $\Gal(\Q(\zeta_g)/\Q) \cong (\Z/g\Z)^\times$, complex conjugation is associated to $-1$. The map Frobenius element corresponding to $\mathfrak{q} \cap \Q(\zeta_g)$ sends $\zeta_g \mapsto \zeta_g^c$.
...
% $\alpha^{1/g}$ is a $g$th root of $\alpha$
...
\begin{theorem}[{\cite[Thm.~20b]{kani2011products}}]\label{thm:kani-20b}
  Suppose $E$ is a CM elliptic curve over an algebraically closed field and $H$ is a finite subgroup scheme of $E$. Then $H$ is an ideal %kernel
...
Then %$\lambda_U$ and $\lambda_V$ are automorphisms of $E^g$ induced by $U$ and $V$, respectively, and
$\lambda_A = \lambda_U \lambda_D \lambda_V$. Since
 ...
 Since $\lambda_V(C^g) = C^g$, we now have
  ...
  $$
  %the quotients of $E^g$ by $(\ker \lambda_A) \cap C^g$ and by $(\ker \lambda_D) \cap C^g$ are weakly isomorphic. The lemma follows.
\end{proof}
...
\end{proposition}
\begin{proof}
  Let $K$ be an imaginary quadratic field %as in Lemma~\ref{lem:silly}, there exist a principal prime $\alpha\sO_K$ lying over $\ell$ 
...
inert in $K$, we have $q_i\ne \ell$ for all $i$. % We may assume $q_i \neq \ell$ for all $i$.
...
\begin{proof}
 ...
 %Let $\ell$ be  a prime not equal to $\car(k)$ that is inert in $K$. There are infinitely many such primes $\ell$. 
...
%The standard form $\sE$ is the unique map satisfying $\sE(u,\Omega v) = u^tv$ for all $u,v \in \R^g$. 
...
\begin{proposition}\label{prop:q-expansion}
  ...
  Then for all $Q\in \sD$ there exists $c_Q \in R$ such that for all % but finitely many
...
% By choosing bases for $S^2(X)$ compatibly for all $A$, we can thus guarantee that the $c_Q$ are independent of $A$. 
  ...
  % \[
  %   \sum_{Q \in \sD} c_Q \prod q_{ij}^{Q_{ij}}.
  % \]
 % where $Q$ ranges over $g \times g$ symmetric, half-integral, positive semidefinite matrices.
  ...
  % \begin{align*}
  %   \sum_{Q \in \sD} c_Q \prod q^{A_{ij}Q_{ij}} &= \sum_{Q \in \sD} c_Q q^{\sum A_{ij}Q_{ij}} \\
  %   &= \sum_{Q \in \sD} c_Q q^{\tr(AQ)}.
  % \end{align*}
\end{proof}
...
\begin{proof}[Proof of Theorem~\ref{thm:curves-dense}]
  ...
  Let $n = \min \{\tr(AQ) \colon Q \in S\}$. By the definition of $C$, there is a unique $Q_0 \in S$ such that $\tr(AQ_0) = n$. % If $k = \C$, one has that the $q$-expansion of $f \circ \psimod$ is $\sum_Q c_Q q^{\tr(AQ)}$. By functoriality, this holds when $k$ is an arbitrary algebraically closed field.
...
%\begin{proof}[Proof of Corollary~\ref{cor:agpol}]
  ...
  % Thus we may replace $\agd$ and $f$ in the statement of the Corollary with $\agphi$ and $f\pi_\phi$.
...
Suppose that $g\in\Z_{>0}$, $R$ is a domain,  $D=(d_1,\ldots,d_g)$ is a polarization type, $d_g$ is invertible in $R$, and $n = d_g$. Define a morphism of $R$-schemes 
  ...
  \end{align*}
  where $e_A$ is the level $n$ Weil pairing on $A$. By ???, there is a polarization $\mu$ on $B$ for which $n\lambda = \psi^\vee\mu\psi$.  
  Set $\pi_D(A,\lambda,L) = (B,\mu)$. The following lemmas show that the polarization $\mu$ has type $D$ (and so $\pi_D$ takes values in $\agd$), and $\pi_D$ is surjective.

  REDO PROOF THAT $\mu$ has type $D$.
\begin{lemma}\label{lem:typeDlem}
The polarization $\mu$ has type $D$.
\end{lemma}
  
\begin{proof}
  Let $e_i = n/d_i$. For each $i$, choose $Q_i' \in A$ for which $e_iQ_i' = Q_i$. Then $B[n]$ is generated by the images of $P_1, \ldots, P_g, Q_i', \ldots, Q_g'$ under $\psi$. Let $P \in A[n]$ and $Q \in B[n]$. Then
  \[
    e_B(\mu\psi(P),Q) = e_A(P,\psi^\vee\mu(Q)).
  \]
  We may write
  \[
    Q = \sum (a_i \psi(P_i) + b_i \psi(Q_i'))
  \]
  for some integers $a_i, b_i$. Then
  \begin{align*}
    e_A(P, \psi^\vee\mu(Q)) &= e_A(P, \psi^\vee\mu\psi(\sum_i a_i P_i + b_iQ_i')) \\
                       &= e_A(P, \lambda n(\sum a_i P_i + b_iQ_i')) \\
                       &= e_A(P, \lambda(\sum b_id_iQ_i)) \\
                       &= e_A^\lambda(P,\sum b_id_iQ_i).
  \end{align*}
...
% \[
%   \Z/d_1 \Z \times \cdots \times \Z/d_g \Z  \cong  \langle \psi(Q_1), \ldots, \psi(Q_g)\rangle =  \ker \rho \subseteq \ker \mu.
%   \]
% Take $P_i' \in A$ such that $nP_i' = P_i$. Let $e_i=n/d_i$. 
%
% We will show that $\psi(e_iP_i') \in \ker \mu$ for all $i$. Since 
% $$
% \mu\psi(e_iP_i') = \rho^\vee \lambda\rho\psi(e_iP_i') = \rho^\vee \lambda(ne_iP_i') = \rho^\vee \lambda(e_iP_i),
% $$
% it suffices to show that
% $e_B(T,\rho^\vee \lambda(e_iP_i))=1$
% for all $T\in B[n]$, where 
% $$
% e_B: B[n] \times B^\vee[n] \to \mu_n
% $$ 
% is the Weil pairing.
% Since $\rho(B[n]) = \ker(\psi)$ and 
% $$e_B(T,\rho^\vee \lambda(e_iP_i))=e_A(\rho(T),\lambda(e_iP_i)),$$
% where $e_A: A[n] \times A^\vee[n] \to \mu_n$ is the Weil pairing, 
% it suffices to show that
% $$e_A(P_j,\lambda(e_iP_i))=1 \quad\text{ and } \quad e_A(d_jQ_j,\lambda(e_iP_i))=1$$
% for all $i$ and $j$. But this follows from the facts that $d_ie_i=n$ and $L$ is a symplectic basis with respect to the pairing on $A[n]$ induced by $e_A$ and $\lambda$.
%
% Letting
% $$
% S = \langle \psi(Q_i), \psi(e_iP_i') : 1\le i \le g \rangle \cong (\Z/d_1 \Z \times \cdots \times \Z/d_g \Z)^2,
% $$
% we now have $S \subseteq \ker \mu$. Since $\deg \mu = (\deg \rho)^2 =(\prod_{i=1}^g d_i)^2$, we have $\ker \mu = S$.
Thus $\mu$ has type $D$, as desired.
\end{proof}

REDO PROOF THAT $\pi_D$ IS SURJECTIVE
...
\begin{proof}
  Suppose $(B,\mu) \in \agd$. Let $\mu': B^\vee \to B$ be the unique isogeny for which $\mu\mu' = [n]$. \textcolor{red}{Is it obvious that $\mu'$ is a polarization?} Letting $e_i = n/d_i$, we have that
\[
  \ker \mu' \cong (\Z/e_1\Z \times \cdots \Z/e_g\Z)^2.
\]
  Fix a subgroup $J$ of $\ker \mu'$ such that $$H \isom  \Z/e_1 \Z \times \cdots \times \Z/e_g \Z$$ and such that
 $J$ is isotropic with respect to the skew-symmetric pairing %$e^\mu$ 
  on $\ker \mu'$ defined on p.~135 of~\cite{milne-av}.
  Let $A^\vee = B^\vee/J$, and write $\psi^\vee$ for the quotient map. Let $A = (A^\vee)^\vee$ and $\psi: A \to B$ the dual isogeny to $\psi^\vee$. There is then a principal polarization $\lambda': A^\vee \to A$ for which $\mu' = \psi\lambda'\psi^\vee$. Let $\lambda = \lambda'^{-1}$. We wish to find a level structure $L$ on $A$ for which $\pi_D(A,\lambda,L) = (B,\mu)$.

  Let $I = \lambda'\psi^\vee(\ker \mu')$. Observe that $I \cong \Z/e_1\Z \times \cdots \times \Z/e_g\Z$. Let $V = A[n]$ with the symplectic form $e_n^\lambda$. In Lemmas~\ref{lemma:I-isotropic} and~\ref{lemma:Iperp-modI-ker-mu} below, we show that $I$ satisfies the conditions of Proposition~\ref{prop:diagonalize-symplectic}. Therefore, by the latter proposition, there is a symplectic basis $P_1, \ldots, P_g, Q_1, \ldots, Q_g$ for $V$ such that
  \[
    I= \langle d_1 Q_1, \ldots, d_g Q_g \rangle.
  \]
  \textcolor{red}{Why does $\pi_D(A,\lambda,L) = (B,\mu)$?}
\end{proof}
...
% \begin{proof}
% Suppose $(B,\mu) \in \agd$.  
%   Fix a subgroup $H$ of $\ker \mu$ %by \cite[Prop.~16.8 and Remark~16.9]{milne-av},
%   such that $$H \isom  \Z/d_1 \Z \times \cdots \times \Z/d_g \Z$$ and such that
%  $H$ is (maximal) isotropic with respect to the perfect skew-symmetric pairing %$e^\mu$ 
%   on $\ker \mu$ defined on p.~293 of \cite{mumford1966}.
%   Let $\rho : B \to B/H$ be the projection map and let $A = B/H$.
%   By \cite[Proposition 1]{mumford1966}, the abelian variety $A$ has a principal polarization $\lambda$ such that $\rho^*(\lambda) = \mu$. \textcolor{red}{We would like to say that} there is a level structure $L$ such that $\pi_D(A, \lambda, L) \cong (B,\mu)$, so $\pi_D$ is surjective. \textcolor{red}{How do we show this? We thought that \cite[Proposition 1]{mumford1966} would help, but we don't see a complete proof.}
%   % Define $\phi: \Z/d_1\Z \times \cdots \times \Z/d_g\Z \to \ker \mu$ by sending the $i$th standard basis vector to $\psi(Q_i)$; since $\psi(Q_i) \in \ker \rho$, it follows that $\psi(Q_i) \in \ker \mu$. By \cite[Proposition 1]{mumford1966}, $\im(\phi)$ is isotropic with respect to $e^\mu$. Set $\pi_D_2(A,\lambda,L) = (B,\mu,\phi)$.
% \end{proof}
...
\[
\mg(k) =  \{(x_1,x_2) \in \agn(k) \times \agn(k) \mid x_1 \medium x_2 \}. %  \subseteq (\agn \times \agn)(k).
\]
...
%  $\pi_D:\agn \longrightarrow \agd$
%\]
%  as follows. If $(A,\lambda,L) \in \agn$ with $L=(P_1,\ldots,P_g,Q_1,\ldots,Q_g)$, let $B$ denote the quotient of $A$ by the subgroup generated by $\{ P_1,\ldots,P_g,d_1Q_1,\ldots,d_gQ_g \}$ and let $\psi: A \to B$ be the quotient map. Since $\ker(\psi) \subseteq A[n]$, there exists an isogeny $\rho: B \to A$ such that the map $\rho \circ \psi$ is multiplication by $n$. Let $\mu$ be the pull-back polarization $\rho^*(\lambda) = \rho^\vee \circ \lambda\circ\rho$. By \cite[Prop.~16.8]{milne-av}, there is a perfect skew-symmetric pairing %$e^\mu$ 
%  on $\ker \mu$ with respect to which $\ker \rho$ is a maximal isotropic subgroup of $\ker \mu$. Since
%  \[
%    \ker \rho = \langle \psi(Q_1), \ldots, \psi(Q_g)\rangle \cong \Z/d_1 \Z \times \cdots \times \Z/d_g \Z,
% \]
%  the polarization $\mu$ has type $D$. Set $\pi_D(A,\lambda,L) = (B,\mu)$.
%  
%  Also by \cite[Prop.~16.8]{milne-av}, if $(B,\mu) \in \agd$, then for every choice of maximal isotropic subgroup $H \subseteq \ker \mu$, the quotient %$\rho: B \to 
%  $B/H$ is an abelian variety that has a principal polarization $\lambda$ such that $\rho^*(\lambda) = \mu$. There is then an appropriate choice of level structure $L$ such that $\pi_D(B/H, \lambda, L) \cong (B,\mu)$, so $\pi_D$ is surjective.
  % Define $\phi: \Z/d_1\Z \times \cdots \times \Z/d_g\Z \to \ker \mu$ by sending the $i$th standard basis vector to $\psi(Q_i)$; since $\psi(Q_i) \in \ker \rho$, it follows that $\psi(Q_i) \in \ker \mu$. By \cite[Proposition 1]{mumford1966}, $\im(\phi)$ is isotropic with respect to $e^\mu$. Set $\pi_D_2(A,\lambda,L) = (B,\mu,\phi)$.
Suppose $(A_1, \lambda_1, L_1) \medium (A_2, \lambda_2, L_2)$ in $\agn$. Let $(B_i, \mu_i) = \pi_D(A_i, \lambda_i, L_i)$. Then $B_i = A_i/H_i$ for some subgroup $H_i \subset A_i[n]$. By the definition of $\medium$, there is a weak isomorphism $A_1 \to A_2$ that sends $H_1$ isomorphically onto $H_2$, and hence %$(B_1, \mu_1) \weak (B_2, \mu_2)$. Therefore
  $B_1 \weak B_2$. Thus
%  \[
    $(\pi_D\times\pi_D)(\mg(k)) \subseteq \sgd(k).$
%  \]
By Proposition~\ref{prop:medium-isomorphic}, the Zariski closure of $\mg(k)$ is $\agn \times \agn$. 
%Therefore the preimage of the closure of $\sgd(k)$ is $\agn \times \agn$.  Since the morphism $\pi_D$ is surjective, $\pi_D \times \pi_D$ is also surjective, and %we have $(\pi_D\times\pi_D)(\mg(k)) = \sgd(k)$.
  %the claim follows.
