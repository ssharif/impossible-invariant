...
Since ${\cal A}_g$ is only a coarse moduli space, one might wonder whether Theorem~\ref{thm:invariant-c-constant} still holds if ${\cal A}_g$ is replaced by the fine moduli space of abelian varieties with full level $n$ structure with $n\ge 3$ and invertible in $R$. The following generalization of 
...
\begin{corollary}
  Let $g \in \Z_{\geq 2}$, and let ${d} = (d_1, \ldots, d_g)$ where the $d_i$ are positive integers satisfying $d_1 \mid \cdots \mid d_g$. Let $\agd$ be the moduli space of abelian varieties with a polarization of type ${d}$. Suppose that $R$ is a domain with $d_g$ invertible in $R$ and suppose $f: \agd \dashrightarrow X$ is a rational map of $R$-schemes. Denote points of $\agd$ as pairs $(A,\lambda)$, where $A$ is an abelian variety and $\lambda$ is a polarization of type ${d}$. Suppose $f((A_1, \lambda_1)) = f((A_2, \lambda_2))$ whenever $A_1 \approx A_2$. Then $f$ is a constant function.
\end{corollary}

\begin{proof}
  Let $n = d_g$. There is a surjective morphism $\pi_d: \agn \to \agd$ defined as follows. ???
\end{proof}
