...
\begin{proof}
If $q$ is an odd prime not dividing an integer $d$, the $q$ splits (respectively, is inert) 
...
\end{itemize}
Clearly there are infintely many such $K$.
If $p=2$ the argument is similar, just replace ``$(\Z/p\Z)^\times$'' by 
``$(\Z/8\Z)^\times$'' in the first congruence.  If $\ell = 2$, replace 
``$(\Z/\ell\Z)^\times$'' by ``$(\Z/8\Z)^\times$'' in the second congruence.
\end{proof}
...
\begin{theorem}\label{thm:curves-dense-a1}
    If $g \in \Z_{\ge 1}$ then there are infinitely many
    %exists a 
    prime numbers $\ell$ not equal to the characteristic of the base ring such that $\bigcup_{A\in W_{\ell,g}} X^{(1)}_A$ is Zariski dense in $\ao^g$.
\end{theorem}
\begin{proof}
 Let $k$ be an algebraically closed field containing the base ring. Let $E_0$ be an elliptic curve over $k$ with CM by $\sO_K$ for some imaginary quadratic field $K$. Let $\ell$ be  a prime not equal to $\car(k)$ that is inert in $K$. There are infinitely many such primes $\ell$. Fix a subgroup $\sC_0$ of $E_0$ of order $\ell$. Let $\pi_0$ denote the quotient map $E_0 \to E_0/\sC_0 =: E_1$. Since $\End(E_0)$ has no ideal of norm $\ell$, $\sC_0$ is not an  ideal subgroup of $E_0$ by Lemma~\ref{lemma:order-ideal-subgroup}. Theorem~\ref{thm:kani-20b} and~\cite[Prop.~5]{kohel1996endomorphism} now imply that $\End(E_1) = \Z + \ell\sO_K$.
...
\begin{proposition}\label{prop:q-expansion}
  ...
  Then for all $Q\in \sD$ there exists $c_Q \in R$ such that for all $A \in \detl$, the $q$-expansion of 
  %the modular form 
