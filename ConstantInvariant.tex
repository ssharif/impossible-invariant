...
\begin{theorem}\label{thm:invariant-c-constant}
  Suppose that $g\in\Z_{\ge 2}$, $R$ is a domain, and $f: \ag \dashrightarrow X$ is a rational map of $R$-schemes. Suppose $f(A_1) = f(A_2)$ whenever $A_1 \approx A_2$. Then $f$ is a constant function.
\end{theorem}
...
\begin{corollary}\label{cor:agn}
  Let $g,n \in \Z_{\geq 2}$, and let $\agn$ be the moduli space of principally polarized abelian varieties with full level $n$ structure. Suppose that $R$ is a domain with $n$ invertible in $R$, and $f: \agn \dashrightarrow X$ is a rational map of $R$-schemes. View points of $\agn$ as pairs $(A,L)$, where $A$ is a principally polarized abelian variety and $L$ is a symplectic basis for the $n$-torsion $A[n]$. Suppose $f((A_1, L_1)) = f((A_2, L_2))$ whenever $A_1 \approx A_2$. Then $f$ is a constant function.
\end{corollary}


Next, we obtain a generalization of Theorem \ref{thm:invariant-c-constant} with $\ag$ replaced by the moduli space of  abelian varieties with a non-principal polarization type. Let $k$ be an algebraically closed field. Recall from~\cite[Theorem 1 et seq.]{mumford1966} that if $\mu: A \to A^\vee$ is a polarization on a $g$-dimensional abelian variety over $k$ of degree not divisible by $\car(k)$, then 
\[
  \ker \mu \cong (\Z/d_1\Z \times \cdots \times \Z/d_g\Z)^2
\]
for some positive integers $d_1 \mid \cdots \mid d_g$, $\car(k) \nmid d_g$. Letting $D = (d_1, \ldots, d_g)$, we say that $D$ is the \emph{type} of $\mu$. 
\begin{corollary}\label{cor:agpol}
  Let $g \in \Z_{\geq 2}$, and let ${D} = (d_1, \ldots, d_g)$ where the $d_i$ are positive integers satisfying $d_1 \mid \cdots \mid d_g$. Let $\agd$ be the moduli space of abelian varieties with a polarization of type ${D}$. Suppose that $R$ is a domain with $d_g$ invertible in $R$ and suppose $f: \agd \dashrightarrow X$ is a rational map of $R$-schemes. View points of $\agd$ as pairs $(A,\mu)$, where $A$ is an abelian variety and $\mu$ is a polarization of type ${D}$. Suppose $f(A_1, \mu_1) = f(A_2, \mu_2)$ whenever $A_1 \approx A_2$. Then $f$ is a constant function.
\end{corollary}
...
Next we derive Theorem \ref{thm:sg-c-dense} from Theorems \ref{thm:Sg-dense} and \ref{thm:curves-dense},  we derive Theorem \ref{thm:invariant-c-constant} from Theorem \ref{thm:sg-c-dense},  we derive Corollary \ref{cor:agn} from Theorem \ref{thm:invariant-c-constant}, and we derive Corollary \ref{cor:agpol} from Corollary \ref{cor:agn}.
...
\begin{proof}[Proof of Corollary~\ref{cor:agpol}]
  Let $n = d_g$. If $\mu$ is a type $D$ polarization on $A$, then by \cite[Theorem 1]{mumford1966}, $\mu$ induces a nondegenerate skew-symmetric bilinear pairing $e^\mu$ on $\ker \mu$. Let $\agphi$ be the moduli space for triples $(A,\mu,\phi)$ where $A$ is an abelian variety, $\mu$ is a type $D$ polarization on $A$, and $\phi$ is an injective homomorphism
  \[
    \phi: \Z/d_1\Z \times \cdots \times \Z/d_g\Z \to \ker \mu
  \]
  for which $\im(\phi)$ is isotropic with respect to $e^\mu$.
  There is a natural forgetful map $\pi_\phi: \agphi \to \agd$ sending $(A,\mu,\phi)$ to $(A,\mu)$. % Thus we may replace $\agd$ and $f$ in the statement of the Corollary with $\agphi$ and $f\pi_\phi$.

  Let $\agn$ be the moduli space for triples $(A,\lambda,L)$, where $\lambda$ is a principal polarization on $A$ and $L$ is a level $n$ structure; that is, a symplectic basis $(P_1,\ldots,P_g,Q_1,\ldots,Q_g)$ for the $n$-torsion $A[n]$. There are maps
  \[
    \agn \stackrel{\pi_2}{\longrightarrow} \agphi \stackrel{\pi_1}{\longrightarrow} \ag
  \]
  defined as follows. For $(A,\lambda,L) \in \agn$, define $B$ as the quotient of $A$ by the subgroup $\langle P_1,\ldots,P_g,d_1Q_1,\ldots,d_gQ_g \rangle$. Let $\psi: A \to B$ be the quotient map. Since $\ker(\psi) \subset A[n]$, there exists an isogeny $\rho: B \to A$ such that $\rho \circ \psi = [n]$, the multiplication-by-$n$ map. Define $\mu$ to be the pull-back polarization $\rho^*(\lambda)$. Define $\phi$ by sending the $i$th standard basis vector to $\psi(Q_i)$. Since $\psi(Q_i) \in \ker \rho$, we see that $\psi(Q_i) \in \ker \mu$. By \cite[Proposition 1]{mumford1966}, $\im(\phi)$ is isotropic for $e^\mu$. Define $\pi_2(A,\lambda,L) = (B,\mu,\phi)$.

  For $\pi_1$, given $(B,\mu,\phi) \in \agphi$, let $A = B/\im(\phi)$. Then by~\cite[Prop. 16.8]{milne-av}, $\mu$ descends to a unique principal polarization $\lambda$ on $A$. Set $\pi_1(B,\mu,\phi) = (A,\lambda)$.
  One checks that $\pi_1\pi_2(A,\lambda,L) = (A,\lambda)$.

  We first assume that $f$ is a morphism.

  Let $\simg$ be the symmetric quotient of $X^G$, where $G$ is the Galois group for $\agn/\ag$. Geometrically closed points of $\simg$ can be viewed as degree $\# G$ zero cycles on $X$. We define a map $f_G: \ag \to \simg$ as follows. For $z \in \ag$, fix $x \in \agn$ for which $\pi_1\pi_2(x) = z$. Define $f_G(z) = \sum_{g \in G} (f\pi_\phi\pi_2)(gx)$.
  ...
  Now suppose $(A_1,\lambda_1), (A_2,\lambda_2) \in \ag$ with $A_1 \approx A_2$. I claim that $f_G(A_1,\lambda_1) = f_G(A_2,\lambda_2)$. To see this, let us first define, for $(A,\lambda) \in \ag$, $S_d(A,\lambda)$ to be the set of pairs $(H_1,H_2)$ where $H_i$ is an isotropic subgroup of $A[n]$ and
  \[
    H_1 \cong (\Z/n\Z)^g, H_2 \cong d_1\Z/n\Z \times \cdots \times d_g\Z/n\Z.
  \]
  Note that our notion of ``isotropic'' depends on the polarization $\lambda$.
  
  Observe that $f_G(A_1,\lambda_1) = \sum f(B_1,\mu_1)$, where in the sum $B_1$ varies over quotients of $A_1$ by subgroups $H_1 + H_2$, $(H_1,H_2) \in S_d(A_1,\lambda_1)$. (Note that each $B_1$ will appear with some multiplicity, which generically depends only on $D$ and $g$.) Similarly, $f_G(A_2) = \sum (B_2, \mu_2)$ where $B_2$ varies over quotients $A_2/(H_1' + H_2')$, $(H_1',H_2') \in S_d(A_2,\lambda_2)$. Fix a weak isomorphism between $A_1$ and $A_2$. Then $\lambda_2^{-1} \circ \lambda_1 \in \Aut(A_1) = \Aut(A_2)$ induces a bijection
  \[
    S_d(A_1,\lambda_1) \to S_d(A_2,\lambda_2).
  \]
  Therefore for each $B_1$ appearing in $f_G(A_1,\lambda_2)$, there is a $B_2$ appearing in $f_G(A_2,\lambda_2)$ for which $B_1 \approx B_2$, and conversely. Since $f$ is constant on weak isomorphism classes, $f_G$ is also. By Theorem~\ref{thm:invariant-c-constant}, $f_G$ is a constant morphism. The space $\agn$ is connected and both $\pi_2$ and $\pi_\phi$ are surjective, so $\agd$ is also connected. Therefore $f$ is constant.

  Suppose instead that $f$ is a rational map. Let $U \subset \agd$ be the domain of definition of $f$. Let $U_n = (\pi_\phi\pi_2)^{-1}(U)$. Let $V_n = \displaystyle\bigcap_{g \in G} g(U_n)$; that is, $V_n$ is the largest Galois-invariant subset of $\agn$ containing $U_n$. Let $V_\phi = \pi_2(V_n)$, $V = \pi_1(V_\phi)$, and $V_D = \pi_\phi(V_\phi)$. The maps $\pi_2, \pi_1, \pi_\phi$ are finite and ???, and hence are open. Therefore $V_\phi, V_D$, and $V$ are open. Replacing $\agn, \agphi, \agd, \ag$ with $V_n, V_\phi, V_d, V$ respectively, yields the diagram
  \[
    \xymatrix{
      V_n \ar[d]^{\pi_2} & & \\
      V_\phi \ar[d]^{\pi_1} \ar[r]^{\pi_\phi} & V_D \ar[r]^f & X \\
      V \ar[rr]^{f_G} & &\simg,
      }
    \]
    where $f_G$ is defined in the same way as above. The rest of the proof then goes through unchanged.
\end{proof}
