...
\begin{remark}
To prove Corollary~\ref{cor:agpol}, we cannot simply apply Corollary~\ref{cor:agn}, since if $A_1 \approx A_2$ and $\pi_2(A_i, \lambda_i,L_i) = (B_i, \mu_i, \phi_i)$, it may not be true that $B_1 \approx B_2$. Furthermore, the reason we used $\agn$ in the proof of Corollary~\ref{cor:agpol} is that while $\agn/\ag$ is Galois, $\agphi/\ag$ is not Galois, so the strategy of the proof of Corollary~\ref{cor:agn} cannot be directly applied.
\end{remark}
