...
\[
  \psi_A^*(f) = \{\tau \mapsto f(A\tau)\}.
\]

Now $\bigcup X_A$ is dense in $\ag$ if and only if for all nonzero Siegel modular functions $f: \ag \to \C$, there is some $\psi_A$ such that $\psi_A^*(f) \neq 0$.

Every Siegel modular function is of the form
\[
  f(T) = \sum_Q c_Q \exp(2 \pi i)^{\tr(QT)}
\]
where $T \in \hh_g$ and the sum is over all quadratic forms $Q$ \textcolor{red}{TODO: do we use quadratic forms instead of symmetric matrices because characteristic $2$ fields?} Then
\[
  \psi_A^*(f)(\tau) = f(A\tau) = \sum_Q c_Q \exp(2 \pi i \tau)^{\tr(AQ)}.
\]
Let $q = \exp(2 \pi i \tau)$. Then we can rewrite the previous equation as
\[
  \psi_A^*(f)(\tau) = \sum_Q c_Q q^{\tr(AQ)}
\]

\begin{lemma}
  For any positive definite symmetric real matrix $A$ and $t \in \R$, the set of positive semidefinite integral quadratic forms $Q$ such that $\tr(AQ) = t$ is finite.
\end{lemma}
\begin{proof}
  Recall that $\langle x,y \rangle = \tr(xy^t)$ gives an inner product on the space of real matrices. Moreover, if $B$ is any positive semidefinite matrix, then $\tr(B^2) \leq \tr(B)^2$. So
  \begin{align*}
  ...
  \]
  There is only finitely many positive semidefinite integral quadratic forms with bounded trace\textcolor{red}{TODO: why}
\end{proof}
