...
The map $\psi_A$ induces a map %descends to a morphism
$$
...
Let
  ...
  \]
  (I don't understand the next sentence.) By Lemma~\ref{lemma:ggt-spd-detl}, we may replace $A$ with $\ell^n GG^t$ with $G \in \Sl_g(Z[1/\ell])$, $n$ sufficiently large (depending on $G$). Since $\det (\ell^n GG^t) = \ell^n$, we have 
  \[
