...
Suppose that $g\in\Z_{>0}$, $R$ is a domain,  $D=(d_1,\ldots,d_g)$ is a polarization type, $d_g$ is invertible in $R$, and $n = d_g$.%  Define a morphism of $R$-schemes 
  ...
  % as follows.
Let $\pi_D: \agn \to \agd$ be the map on moduli spaces induced by the morphism of stacks $f$ defined on the bottom of p.~492 of ??cite[de Jong].
...
%  $J$ is (maximal) isotropic with respect to the perfect skew-symmetric pairing %$e^\mu$ 
...
%   % Choose a basis $U_1, \ldots, U_{2g}$ for $B^\vee[n]$ such that $\ker \psi^\vee$ is generated by $d_1U_{g+1}, \ldots, d_g U_{2g}$. For $1 \leq i \leq g$, let $V_i \in B^\vee$ satisfy $e_iV_i = U_{i+g}$. Then $A[n]$ is generated by $\lambda'\psi^\vee(U_1), \ldots, \lambda'\psi^\vee(U_g), \lambda'\psi^\vee(V_1), \ldots, \lambda'\psi^\vee(V_g)$. Then for $1 \leq i \leq g$, 
%   % \begin{align*}
%   %   (\psi^\vee\mu\psi)(\lambda'\psi^\vee)(U_i) &= (\psi^\vee\mu \mu')(U_i) \\
%   %                                        &= \psi^\vee(nU_i) \\
%   %                                        &= 0.
%   % \end{align*}
%   % Similarly,
%   % \[
%   %   (\psi^\vee\mu\psi)(\lambda'\psi^\vee)(V_i) = \psi^\vee(d_iU_{i+g}) = 0.
%   % \]
%   % Thus $\psi^\vee\mu\psi(A[n]) = 0$. But the degree of $\psi^\vee\mu\psi$ is $n^{2g}$, and hence $\psi^\vee\mu\psi = n \lambda$.
% \end{proof}
...
% \end{proof}
 ...
 Define an equivalence relation $\medium$ on $\agn$ by $(A_1, \lambda_1, L_1) \medium (A_2, \lambda_2, L_2)$ if and only if $A_1 \weak A_2$ and there is an isomorphism $\phi: A_1 \to A_2$ such that $\phi(L_1) = L_2$. If $k$ is an algebraically closed field that contains the base ring $R$, define a set $\mg(k)  \subseteq (\agn \times \agn)(k)$ by
\[
...
%  $B/H$ is an abelian variety that has a principal polarization $\lambda$ such that $\rho^*(\lambda) = \mu$. There is then an appropriate choice of level structure $L$ such that $\pi_D(B/H, \lambda, L) \cong (B,\mu)$, so $\pi_D$ is surjective.
  % Define $\phi: \Z/d_1\Z \times \cdots \times \Z/d_g\Z \to \ker \mu$ by sending the $i$th standard basis vector to $\psi(Q_i)$; since $\psi(Q_i) \in \ker \rho$, it follows that $\psi(Q_i) \in \ker \mu$. By \cite[Proposition 1]{mumford1966}, $\im(\phi)$ is isotropic with respect to $e^\mu$. Set $\pi_D_2(A,\lambda,L) = (B,\mu,\phi)$.
Suppose $(A_1, \lambda_1, L_1) \medium (A_2, \lambda_2, L_2)$ in $\agn$. Let $(B_i, \mu_i) = \pi_D(A_i, \lambda_i, L_i)$. From the definition of $f$, $B_i^\vee = A_i^\vee/H_i$ for some subgroup $H_i \subset A^\vee_i[n]$. By the definition of $\medium$, there is a weak isomorphism $A_1 \to A_2$ whose dual sends $H_1$ isomorphically onto $H_2$, and hence %$(B_1, \mu_1) \weak (B_2, \mu_2)$. Therefore
...
Let $Z$ be the Zariski closure of $(\pi_D \times \pi_D)(\mg(k))$. Let $\xi$ be a geometric generic point for $Z$. By the argument preceding ??cite[Prop. 1.11]{de Jong} and the universal property of the coarse moduli space associated to a stack, there is a geometric generic point $\xi'$ of $\agn \times \agn$ for which $\pi_D(\xi') = \xi$. But the closure of $\xi'$ must contain $\mg(k)$, and hence $\xi'$ is the geometric generic point for $\agn \times \agn$ itself. By ??cite[Prop. 1.11]{de Jong}, $\agd$ is irreducible, and hence $\xi$ must be the geometric generic point of $\agd \times \agd$. Therefore the Zariski closure of $(\pi_D \times \pi_D)(\mg(k))$ is $\agd \times \agd$.
\end{proof}
