...
\begin{proof}[Proof of Corollary~\ref{cor:agpol}]
  ...
  \]
  defined as follows. For $(A,\lambda,L) \in \agn$, define $B$ as the quotient of $A$ by the subgroup $\langle P_1,\ldots,P_g,d_1Q_1,\ldots,d_gQ_g \rangle$. Let $R_i$ be the image of $Q_i$ in $B$. Let $\rho: B \to A$ be the dual isogeny, and define $\lambda'$ to be the pull-back polarization $\rho^*\lambda$. Define $\phi$ by sending the $i$th standard basis vector to $R_i$. Since $R_i \in \ker \rho$, we see that $R_i \in \ker \lambda$. Define $\pi_2(A,\lambda,L) = (B,\lambda',\phi)$.
