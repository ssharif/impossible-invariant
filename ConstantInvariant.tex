...
Next, we obtain a generalization of Theorem \ref{thm:invariant-c-constant} with $\ag$ replaced by the moduli space of  abelian varieties with a non-principal polarization type.
\begin{corollary}\label{cor:agpol}
  Let $g \in \Z_{\geq 2}$, and let ${d} = (d_1, \ldots, d_g)$ where the $d_i$ are positive integers satisfying $d_1 \mid \cdots \mid d_g$. Let $\agd$ be the moduli space of abelian varieties with a polarization of type ${d}$. Suppose that $R$ is a domain with $d_g$ invertible in $R$ and suppose $f: \agd \dashrightarrow X$ is a rational map of $R$-schemes. View points of $\agd$ as pairs $(A,\lambda)$, where $A$ is an abelian variety and $\lambda$ is a polarization of type ${d}$. Suppose $f((A_1, \lambda_1)) = f((A_2, \lambda_2))$ whenever $A_1 \approx A_2$. Then $f$ is a constant function.
\end{corollary}
...
\begin{proof}[Proof of Corollary~\ref{cor:agpol}]
  ...
  \]
  There is a natural forgetful map $\pi_\phi: \agphi \to \agd$. Observe that if $A_1 \approx A_2$, then $(f\pi_\phi)(A_1,\lambda_1,\phi_1) = (f\pi_\phi)(A_2,\lambda_2,\phi_2)$. Thus we may replace $\agd$ and $f$ in the statement of the Corollary with $\agphi$ and $f\pi_\phi$. We first assume that $f$ is a morphism.
  ...
  Let $\simg$ be the symmetric quotient of $X^G$, where $G$ is the Galois group for $\agn/\ag$. Geometrically closed points of $\simg$ can be viewed as degree $\# G$ zero cycles on $X$. We define a map $f_G: \ag \to \simg$ as follows. For $z \in \ag$, fix $y \in \agn$ for which $\pi(y) = x$. Define $f_G(z) = \sum_{g \in G} (f\pi_2)(gy)$.
  ...
  Observe that $f_G(A_1,\lambda_1) = \sum (B_1,\lambda_1',\phi_1)$, where in the sum the $B_1$ vary over quotients of $A_1$ by subgroups $H_1 + H_2$, $(H_1,H_2) \in S_d(A_1,\lambda_1)$. (Note that each $B_1$ will appear with some multiplicity, which generically depends only on $d$ and $g$.) Similarly, $f_g(A_2) = \sum (B_2, \lambda_2', \phi_2)$ where the $B_2$ varies over quotients $A_2/(H_1' + H_2')$, $(H_1',H_2') \in S_d(A_2,\lambda_2)$. Fix a weak isomorphism between $A_1$ and $A_2$. Then $\lambda_2^{-1} \circ \lambda_1 \in \End(A_1) = \End(A_2)$ induces a bijection
  \[
