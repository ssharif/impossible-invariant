...
\begin{proof}[Proof of Corollary~\ref{cor:agpol}]
  ...
  Let $\sg(X)$ be the symmetric quotient of $X^G$, where $G$ is the Galois group for $\agn/\ag$. Geometrically closed points of $\sg(X)$ can be viewed as degree $\# G$ zero cycles on $X$. We define a map $f_G: \ag \to \sg(X)$ as follows. For $z \in \ag$, fix $y \in \agn$ for which $\pi(y) = x$. Define $f_G(z) = \sum_{g \in G} (f\pi_2)(gy)$.
