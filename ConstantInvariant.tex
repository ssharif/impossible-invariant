...
\begin{lemma}\label{lemma:q-expansion}
  Suppose $f$ is a modular form on $\ag$, and $A$ is a $g \times g$ symmetric, positive definite integral matrix with determinant $N \neq 0$. Then at any cusp of $X_0(N)$, the $q$-expansion of the modular form $\psimod^*(f)$ is of the form
  \[
    \sum_Q c_Q q^{\tr(AQ)},
  \]
...
\begin{proof}
  Let $Y = \Z$ be the character group of the torus $\G_m$, and $X = \Z^g$ the character group of the torus $\G_m^g$. Let $q_{ij}$, $1 \leq i \leq j \leq g$, be the standard basis for $S^2(X)$, the symmetric quadratic forms on $X$, and $q$ the standard basis for $S^2(Y)$. Set $q_{ji} = q_{ij}$. As in \cite[Section V.1]{faltings1990degeneration}, a $q$-expansion for $f$ is of the form
  \[
    \sum_Q c_Q \prod q_{ij}^{Q_{ij}},
  \]
  where $Q$ ranges over $g \times g$ symmetric, half-integral, positive semidefinite matrices.

  The morphism $\psimod$ induces a morphism on the toroidal compactifications $X_0(N) \to \overline{\ag}$. We consider $\psimod$ in a formal neighborhood of the $I_1$ cusp of $X_0(N)$. % Here, $\Delta: \G_m \to \G_m^g$ is the diagonal map, and $\lambda_A: \G_m^g \to \G_m^g$ is the natural map induced by $A$.
...
\begin{proof}[Proof of Theorem~\ref{thm:curves-dense}]
  ...
  Let $f$ be a Siegel modular function on $\ag$, viewed as a global section of the structure sheaf. By Lemma~\ref{lemma:q-expansion}, $f$ admits a $q$-expansion of the form $f = \sum_Q c_Q q^{\tr(Q)}$ where $Q$ ranges over thet set $\mathfrak{Q}$ of all symmetric half-integral positive semi-definite matrices. % \cite[Prop. V.1.5]{faltings1990degeneration}.
  ...
  with modular morphisms as expected, we have that the $q$-expansion of $\psimod^*(f)$ is $\sum_Q c_Q q^{\tr(AQ)}$.   By construction, the coefficient of $q^n$ is $c_{Q_0} \neq 0$. Hence $\psimod^*(f) \neq 0$.
\end{proof}
