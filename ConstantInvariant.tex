...
\begin{proof}[Proof of Theorem~\ref{thm:invariant-c-constant}]
  ...
  Let $k$ be an algebraically closed field containing $R$.
  Applying the universal property with $W = \Spec k$ shows that $\df(k)$ consists of the pairs $(A,B)$ of abelian varieties over $k$ such that $f(A) = f(B)$.
  By the hypothesis on $f$, if $A \approx B$ then $f(A) = f(B)$. Hence $\sg(k) \subseteq \df(k)$.
