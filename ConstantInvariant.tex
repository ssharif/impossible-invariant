...
Let $\agn$ denote the moduli space for triples $(A,\lambda,L)$, where $A$ is a $g$-dimensional abelian variety, $\lambda$ is a principal polarization on $A$, and $L$ is a level $n$ structure, i.e., a symplectic basis $(P_1,\ldots,P_g,Q_1,\ldots,Q_g)$ for the $n$-torsion $A[n]$. Define an equivalence relation $\medium$ on $\agn$ by $(A_1, \lambda_1, L_1) \medium (A_2, \lambda_2, L_2)$, if $A_1 \weak A_2$ and there is a choice of weak isomorphism $\phi: A_1 \to A_2$ such that $\phi(L_1) = L_2$. Define $\mg(k) \subset (\agn \times \agn)(k)$ as
\[
  \{(x_1,x_2) \mid x_1 \medium x_2 \}.
\]
\begin{proposition}\label{prop:medium-isomorphic}
  If $g\in\Z_{\ge 2}$, $R$ is a domain, $n$ is an integer which is invertible in $R$, $k$ is an algebraically closed field, and $R \subseteq k$, then the set $\mg(k)$ is Zariski dense in the $R$-scheme $\agn \times \agn$.
\end{proposition}

\begin{proof}
  Let $\pi: \agn \to \ag$ be the forgetful map $(A,\lambda,L) \mapsto (A,\lambda)$. Certainly $\pi(\mg(k)) = \sg(k)$. By Theorem~\ref{thm:sg-c-dense}, $\sg(k)$ is Zariski dense in $\ag \times \ag$, and so the dimension of the Zariski closure of $\sg(k)$ is $2 \dim \ag$. Therefore the dimension of the Zariski closure of $\mg(k)$ is also $2 \dim \ag = 2 \dim \agn$. Since $\agn$ is connected, $\agn \times \agn$ is also connected, and the claim follows.
\end{proof}

Define $\sgd(k) \subset \agd \times \agd$ as
\[
  \{((B_1,\mu_1), (B_2, \mu_2)) \mid B_1 \weak B_2 \}.
\]
Using the notation in Corollary~\ref{cor:agpol}, let $n = d_g$.  We define a morphism %  morphisms %maps
  % \[
...
\]
\begin{theorem}
  Suppose that $g\in\Z_{\ge 2}$, $R$ is a domain, and $f: \alld \dashrightarrow X$ is a rational map of $R$-schemes. Suppose $f(A_1,\lambda_1) = f(A_2,\lambda_2)$ whenever $A_1 \weak A_2$ and $(A_1,\lambda_1)$ and $(A_2,\lambda_2)$ are in the domain of definition of $f$. Then $f$ is a constant function.
\end{theorem}

\begin{proof}
  By Corollary~\ref{cor:agpol}, $f$ is constant on each $\agd$. Consider two polarization types $D, D'$. We wish to show that $f(\agd) = f(\agdp)$. Choose an elliptic curve defined over $R$. Then $E^g$ has polarizations $\mu, \mu'$ of types $D$ and $D'$ respectively over an appropriate $R$-algebra $S$. Thus $(E^g, \mu) \in \agd(S)$ and $(E^g,\mu') \in \agdp(S)$. But certainly $E^g \weak E^g$, and so $f(E^g, \mu) = f(E^g, \mu')$. Therefore $f(\agd) = f(\agdp)$. \textcolor{red}{What if $E^g$ is not in domain of definition?}
\end{proof}
