...
Suppose $(B,\mu) \in \agd$.  
 ...
 $H$ is (maximal) isotropic with respect to the perfect skew-symmetric pairing %$e^\mu$ 
  ...
  Let $\rho : B \to B/H$ be the projection map.
  By \cite[Proposition 1]{mumford1966}, the quotient $B/H$ is an abelian variety that has a principal polarization $\lambda$ such that $\rho^*(\lambda) = \mu$. By \cite[Proposition 1]{mumford1966}, there is then an appropriate choice of level structure $L$ such that $\pi_D(B/H, \lambda, L) \cong (B,\mu)$, so $\pi_D$ is surjective.
