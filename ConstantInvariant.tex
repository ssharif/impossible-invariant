...
%\begin{proof}[Proof of Corollary~\ref{cor:agpol}]
  Using the notation in Corollary~\ref{cor:agpol}, let $n = d_g$. If $\mu$ is a type $D$ polarization on $A$, then $\mu$ induces a nondegenerate skew-symmetric bilinear pairing $e^\mu$ on $\ker \mu$, by \cite[Theorem 1]{mumford1966}. Let $\agphi$ denote the moduli space for triples $(A,\mu,\phi)$ where $A$ is a $g$-dimensional abelian variety, $\mu$ is a type $D$ polarization on $A$, and $\phi$ is an injective homomorphism
  \[
    \phi: \Z/d_1\Z \times \cdots \times \Z/d_g\Z \to \ker \mu
  \]
  such that $\im(\phi)$ is isotropic with respect to $e^\mu$.
  Let $\pi_\phi: \agphi \to \agd$ be the forgetful map that sends $(A,\mu,\phi)$ to $(A,\mu)$. % Thus we may replace $\agd$ and $f$ in the statement of the Corollary with $\agphi$ and $f\pi_\phi$.

  Let $\agn$ denote the moduli space for triples $(A,\lambda,L)$, where $A$ is a $g$-dimensional abelian variety, $\lambda$ is a principal polarization on $A$, and $L$ is a level $n$ structure, i.e., a symplectic basis $(P_1,\ldots,P_g,Q_1,\ldots,Q_g)$ for the $n$-torsion $A[n]$. We define maps
  \[
    \agn \stackrel{\pi_2}{\longrightarrow} \agphi \stackrel{\pi_1}{\longrightarrow} \ag
  \]
   as follows. If $(A,\lambda,L) \in \agn$, let $B$ denote the quotient of $A$ by the subgroup generated by $\{ P_1,\ldots,P_g,d_1Q_1,\ldots,d_gQ_g \}$. Let $\psi: A \to B$ be the quotient map. Since $\ker(\psi) \subseteq A[n]$, there exists an isogeny $\rho: B \to A$ such that the map $\rho \circ \psi$ is multiplication-by-$n$. Define $\mu$ to be the pull-back polarization $\rho^*(\lambda)$. Define $\phi: \Z/d_1\Z \times \cdots \times \Z/d_g\Z \to \ker \mu$ by sending the $i$th standard basis vector to $\psi(Q_i)$. Since $\psi(Q_i) \in \ker \rho$, it follows that $\psi(Q_i) \in \ker \mu$. By \cite[Proposition 1]{mumford1966}, $\im(\phi)$ is isotropic with respect to $e^\mu$. Let $\pi_2(A,\lambda,L) = (B,\mu,\phi)$.

  To define the map $\pi_1$, if $(B,\mu,\phi) \in \agphi$, let $A = B/\im(\phi)$. Then $\mu$ descends to a unique principal polarization $\lambda$ on $A$, by~\cite[Prop. 16.8]{milne-av}. Set $\pi_1(B,\mu,\phi) = (A,\lambda)$.
  One checks that $\pi_1\pi_2(A,\lambda,L) = (A,\lambda)$.

  We first assume that $f$ is a morphism.

  Let $G$ denote the Galois group for $\agn$ over $\ag$, and let $\simg$ be the symmetric quotient of $X^G$. Geometrically closed points of $\simg$ can be viewed as degree $\# G$ zero cycles on $X$. We define a map $f_G: \ag \to \simg$ as follows. For $z \in \ag$, fix $x \in \agn$ such that $\pi_1\pi_2(x) = z$. Define $f_G(z) = \sum_{g \in G} (f\pi_\phi\pi_2)(gx)$.
  ...
  Now suppose $(A_1,\lambda_1), (A_2,\lambda_2) \in \ag$ with $A_1 \approx A_2$. We we show that $f_G(A_1,\lambda_1) = f_G(A_2,\lambda_2)$. If $(A,\lambda) \in \ag$, let $S_d(A,\lambda)$ denote the set of pairs $(H_1,H_2)$ where $H_i$ is an isotropic subgroup of $A[n]$, 
  \[
    H_1 \cong (\Z/n\Z)^g, \text{ and } H_2 \cong d_1\Z/n\Z \times \cdots \times d_g\Z/n\Z.
  \]
  Note that our notion of ``isotropic'' depends on the polarization $\lambda$.
  
  Observe that $f_G(A_1,\lambda_1) = \sum f(B_1,\mu_1)$, where in the sum $B_1$ varies over quotients of $A_1$ by subgroups $H_1 + H_2$ such that $(H_1,H_2) \in S_d(A_1,\lambda_1)$. (Each $B_1$ appears with some multiplicity, which generically depends only on $D$ and $g$.) Similarly, $f_G(A_2) = \sum (B_2, \mu_2)$ where $B_2$ varies over quotients $A_2/(H_1' + H_2')$ such that $(H_1',H_2') \in S_d(A_2,\lambda_2)$. Since $\lambda_2^{-1} \circ \lambda_1 \in \Aut(A_1) = \Aut(A_2)$,  a weak isomorphism between $A_1$ and $A_2$ induces a bijection
from $S_d(A_1,\lambda_1)$ to $S_d(A_2,\lambda_2)$.
  Thus for each $B_1$ appearing in the formal sum $f_G(A_1,\lambda_2)$, there is a $B_2$ appearing in $f_G(A_2,\lambda_2)$ for which $B_1 \approx B_2$, and conversely. Since $f$ is constant on weak isomorphism classes, so is $f_G$. By Theorem~\ref{thm:invariant-c-constant}, $f_G$ is a constant morphism. Since $\agn$ is connected and both $\pi_2$ and $\pi_\phi$ are surjective, it follows that $\agd$ is also connected. Thus $f$ is constant.

  Suppose now that $f$ is a rational map, let $U \subseteq \agd$ be its domain of definition, and let $U_n = (\pi_\phi\pi_2)^{-1}(U)$. Let $V_n = \displaystyle\bigcap_{g \in G} g(U_n)$ that is, $V_n$ is the largest Galois-invariant subset of $\agn$ containing $U_n$. Let $V_\phi = \pi_2(V_n)$, let $V = \pi_1(V_\phi)$, and let $V_D = \pi_\phi(V_\phi)$. The maps $\pi_2, \pi_1, \pi_\phi$ are finite and ???, and hence are open. Therefore $V_\phi, V_D$, and $V$ are open. Replacing $\agn, \agphi, \agd, \ag$ with $V_n, V_\phi, V_d, V$, respectively, yields the diagram:
  \[
    \xymatrix{
      V_n \ar[d]^{\pi_2} & & \\
    ...
    \]
    where $f_G$ is defined in the same way as above. The rest of the proof goes through unchanged from the case where $f$ is a morphism.
%\end{proof}
