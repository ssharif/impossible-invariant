...
\begin{theorem}\label{thm:invariant-c-constant}
  Suppose $f: \ag \to X$ be a morphism of schemes over $R$, where $R = \Z, \Q,$ or $\C$. If $f(A) = f(B)$ whenever $A \approx B$, then $f$ is constant.
\end{theorem}
Since a scheme over $\C$ is also scheme over $\Q$ (and $\Z$), it suffices to prove the theorem with $R = \C$. We will deduce the theorem as a corollary of another result, stated below.
