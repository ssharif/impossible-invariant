...
With $\agd$ defined as in Corollary \ref{cor:agpol}, let
%\[
  $\alld = \bigcup_{D} \agd$,
%\]
...
\begin{corollary}
\label{cor:alldcor}
  Suppose that $g\in\Z_{\ge 2}$, $R$ is a domain, and $f: \alld \to X$ is a morphism of $R$-schemes. Suppose $f(A_1,\lambda_1) = f(A_2,\lambda_2)$ whenever $A_1 \weak A_2$. Then $f$ is a constant function.
\end{corollary}
...
\begin{proof}[Proof of Corollary~\ref{cor:alldcor}]
  By Corollary~\ref{cor:agpol}, $f$ is constant on each $\agd$. %Thus we can naturally extend $f$ to a morphism on $\alld$. 
